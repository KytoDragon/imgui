...
import backends_d.imgui_impl_opengl3_loader;

enum IMGUI_IMPL_OPENGL_ES2 = false;
enum IMGUI_IMPL_OPENGL_ES3 = false;
enum IMGUI_IMPL_OPENGL_LOADER_CUSTOM = false;
...
static if (IMGUI_IMPL_OPENGL_ES2) {
    ...
    IM_ASSERT(cast(int)(glsl_version.length) + 2 < IM_ARRAYSIZE(bd.GlslVersionString));
    strcpy(bd.GlslVersionString.ptr, glsl_version.ptr);
    strcat(bd.GlslVersionString.ptr, "\n");
...
static if (IMGUI_IMPL_OPENGL_MAY_HAVE_EXTENSIONS) {
...
static if (IMGUI_IMPL_HAS_POLYGON_MODE) {
...
static if (IMGUI_IMPL_OPENGL_MAY_HAVE_BIND_SAMPLER) {
    GLuint last_sampler; if (bd.GlVersion >= 330 || bd.GlProfileIsES3) { glGetIntegerv(GL_SAMPLER_BINDING, cast(GLint*)&last_sampler); } else { last_sampler = 0; }
}
