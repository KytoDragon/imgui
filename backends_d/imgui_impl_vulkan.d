...
*/
static uint32_t[324] __glsl_shader_vert_spv =
[
...
*/
static uint32_t[193] __glsl_shader_frag_spv =
[
...
{
    return ImGui.GetCurrentContext() ? cast(ImGui_ImplVulkan_Data*)ImGui.GetIO().BackendRendererUserData : null;
}
...
static void ImGui_ImplVulkan_SetupRenderState(ImDrawData* draw_data, VkPipeline pipeline, VkCommandBuffer command_buffer, ImGui_ImplVulkanH_FrameRenderBuffers* rb, int fb_width, int fb_height)
{
    ...
    // Our visible imgui space lies from draw_data->DisplayPps (top left) to draw_data->DisplayPos+data_data->DisplaySize (bottom right). DisplayPos is (0,0) for single viewport apps.
    {
        ...
        vkCmdPushConstants(command_buffer, bd.PipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, sizeof!(float) * 0, sizeof!(float) * 2, scale.ptr);
        vkCmdPushConstants(command_buffer, bd.PipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, sizeof!(float) * 2, sizeof!(float) * 2, translate.ptr);
    }
}
...
void ImGui_ImplVulkan_RenderDrawData(ImDrawData* draw_data, VkCommandBuffer command_buffer, VkPipeline pipeline = VK_NULL_HANDLE)
{
    ...
    for (int n = 0; n < draw_data.CmdListsCount; n++)
    {
        ...
        for (int cmd_i = 0; cmd_i < cmd_list.CmdBuffer.Size; cmd_i++)
        {
            ...
            else
            {
                ...
                VkDescriptorSet[1] desc_set = [ cast(VkDescriptorSet)pcmd.TextureId ];
    ...
    VkRect2D scissor = { { 0, 0 }, { cast(uint32_t)fb_width, cast(uint32_t)fb_height } };
    vkCmdSetScissor(command_buffer, 0, 1, &scissor);
}
...
bool ImGui_ImplVulkan_CreateFontsTexture(VkCommandBuffer command_buffer)
{
    ...
    io.Fonts.SetTexID(cast(ImTextureID)bd.FontDescriptorSet);
...
static void ImGui_ImplVulkan_CreatePipeline(VkDevice device, const (VkAllocationCallbacks)* allocator, VkPipelineCache pipelineCache, VkRenderPass renderPass, VkSampleCountFlagBits MSAASamples, VkPipeline* pipeline, uint32_t subpass)
{
    ...
    dynamic_state.dynamicStateCount = cast(uint32_t)IM_ARRAYSIZE(dynamic_states);
...
bool    ImGui_ImplVulkan_LoadFunctions(PFN_vkVoidFunction function(string function_name, void* user_data) loader_func, void* user_data)
{
...
bool    ImGui_ImplVulkan_Init(ImGui_ImplVulkan_InitInfo* info, VkRenderPass render_pass)
{
    ...
    ImGui_ImplVulkan_Data* bd = IM_NEW!(ImGui_ImplVulkan_Data)(false);
