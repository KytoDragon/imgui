...
void TableBeginInitMemory(ImGuiTable* table, int columns_count)
{
    ...
    const int columns_bit_array_size = cast(int)ImBitArrayGetStorageSizeInBytes(columns_count);
    ...
    table.EnabledMaskByDisplayOrder = cast(ImU32*)span_allocator.GetSpanPtrBegin(3);
    table.EnabledMaskByIndex = cast(ImU32*)span_allocator.GetSpanPtrBegin(4);
    table.VisibleMaskByIndex = cast(ImU32*)span_allocator.GetSpanPtrBegin(5);
}
...
void TableUpdateLayout(ImGuiTable* table)
{
    ...
    for (int column_n = 0; column_n < table.ColumnsCount; column_n++)
    {
        ...
        if (column.Flags & ImGuiTableColumnFlags.WidthFixed)
        {
            ...
            else if ((column.Flags & ImGuiTableColumnFlags.WidthFixed) && !column_is_resizable && column.IsRequestOutput)
                column.WidthRequest = width_auto;
...
void TableMergeDrawChannels(ImGuiTable* table)
{
    ...
    const int size_for_masks_bitarrays_one = cast(int)ImBitArrayGetStorageSizeInBytes(max_draw_channels);
    ...
    for (int n = 0; n < IM_ARRAYSIZE(merge_groups); n++)
        merge_groups[n].ChannelsMask = cast(ImBitArrayPtr)cast(void*)(g.TempBuffer.Data + (size_for_masks_bitarrays_one * n));
    ImBitArrayPtr remaining_mask = cast(ImBitArrayPtr)cast(void*)(g.TempBuffer.Data + (size_for_masks_bitarrays_one * 4));
