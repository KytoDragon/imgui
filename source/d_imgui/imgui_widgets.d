...
void TextColoredV(const ImVec4/*&*/ col, string fmt, va_list args)
{
    ...
    if (fmt == "%s")
    {
        IM_ASSERT(has_string_like_value(args));
...
bool Checkbox(string label, bool* v)
{
    ...
    bool mixed_value = (window.DC.ItemFlags & ImGuiItemFlags.MixedValue) != 0;
    if (mixed_value)
    {
...
bool CheckboxFlags(string label, uint* flags, uint flags_value)
{
    ...
    if (v == false && (*flags & flags_value) != 0)
    {
        ...
        window.DC.ItemFlags |= ImGuiItemFlags.MixedValue;
...
{
    const int a_section = (a.Flags & ImGuiTabItemFlags.Leading) ? 0 : (a.Flags & ImGuiTabItemFlags.Trailing) ? 2 : 1;
    const int b_section = (b.Flags & ImGuiTabItemFlags.Leading) ? 0 : (b.Flags & ImGuiTabItemFlags.Trailing) ? 2 : 1;
...
void TabBarLayout(ImGuiTabBar* tab_bar)
{
    ...
    for (int tab_src_n = 0; tab_src_n < tab_bar.Tabs.Size; tab_src_n++)
    {
        ...
        tab.IndexDuringLayout = cast(ImS8)tab_dst_n;

        // We will need sorting if tabs have changed section (e.g. moved from one of Leading/Central/Trailing to another)
        int curr_tab_section_n = (tab.Flags & ImGuiTabItemFlags.Leading) ? 0 : (tab.Flags & ImGuiTabItemFlags.Trailing) ? 2 : 1;
        if (tab_dst_n > 0)
        {
            ImGuiTabItem* prev_tab = &tab_bar.Tabs[tab_dst_n - 1];
            int prev_tab_section_n = (prev_tab.Flags & ImGuiTabItemFlags.Leading) ? 0 : (prev_tab.Flags & ImGuiTabItemFlags.Trailing) ? 2 : 1;
    ...
    for (int tab_n = 0; tab_n < tab_bar.Tabs.Size; tab_n++)
    {
        ...
        int section_n = (tab.Flags & ImGuiTabItemFlags.Leading) ? 0 : (tab.Flags & ImGuiTabItemFlags.Trailing) ? 2 : 1;
    ...
    if ((tab_bar.WidthAllTabsIdeal > tab_bar.BarRect.GetWidth() && tab_bar.Tabs.Size > 1) && !(tab_bar.Flags & ImGuiTabBarFlags.NoTabListScrollingButtons) && (tab_bar.Flags & ImGuiTabBarFlags.FittingPolicyScroll))
        if (ImGuiTabItem* scroll_track_selected_tab = TabBarScrollingButtons(tab_bar))
        {
            ...
            if (!(scroll_track_selected_tab.Flags & ImGuiTabItemFlags.Button))
    ...
    if (scroll_track_selected_tab_id)
        if (ImGuiTabItem* scroll_track_selected_tab = TabBarFindTabByID(tab_bar, scroll_track_selected_tab_id))
            TabBarScrollToTab(tab_bar, scroll_track_selected_tab, sections.ptr);
...
void TabBarScrollToTab(ImGuiTabBar* tab_bar, ImGuiTabItem* tab, ImGuiTabBarSection* sections)
{
    if (tab.Flags & (ImGuiTabItemFlags.Leading | ImGuiTabItemFlags.Trailing))
        return;
...
bool TabBarProcessReorder(ImGuiTabBar* tab_bar)
{
    ...
    if (tab2.Flags & ImGuiTabItemFlags.NoReorder)
        return false;
    if ((tab1.Flags & (ImGuiTabItemFlags.Leading | ImGuiTabItemFlags.Trailing)) != (tab2.Flags & (ImGuiTabItemFlags.Leading | ImGuiTabItemFlags.Trailing)))
        return false;
...
bool    TabItemEx(ImGuiTabBar* tab_bar, string label, bool* p_open, ImGuiTabItemFlags flags)
{
    ...
    // Layout
    const bool is_central_section = (tab.Flags & (ImGuiTabItemFlags.Leading | ImGuiTabItemFlags.Trailing)) == 0;
    size.x = tab.Width;
