    ...
    if (hovered)
    {
        if (!(flags & ImGuiButtonFlags.NoKeyModifiers) || (!g.IO.KeyCtrl && !g.IO.KeyShift && !g.IO.KeyAlt))
        {
            ...
            if ((flags & ImGuiButtonFlags.PressedOnRelease) && mouse_button_released != -1)
            {
                ...
                if (!((flags & ImGuiButtonFlags.Repeat) && g.IO.MouseDownDurationPrev[mouse_button_released] >= g.IO.KeyRepeatDelay))
                    pressed = true;
                ClearActiveID();
            }
...
bool ImageButton(ImTextureID user_texture_id, const ImVec2/*&*/ size, const ImVec2/*&*/ uv0 = ImVec2(0,0), const ImVec2/*&*/ uv1 = ImVec2(1,1), int frame_padding = -1, const ImVec4/*&*/ bg_col = ImVec4(0,0,0,0), const ImVec4/*&*/ tint_col = ImVec4(1,1,1,1))
{
    ...
    const ImVec2 padding = (frame_padding >= 0) ? ImVec2(cast(float)frame_padding, cast(float)frame_padding) : style.FramePadding;
    const ImRect bb = ImRect(window.DC.CursorPos, window.DC.CursorPos + size + padding * 2);
    const ImRect image_bb = ImRect(window.DC.CursorPos + padding, window.DC.CursorPos + padding + size);
    ItemSize(bb);
    if (!ItemAdd(bb, id))
        return false;
    ...
    const ImU32 col = GetColorU32((held && hovered) ? ImGuiCol.ButtonActive : hovered ? ImGuiCol.ButtonHovered : ImGuiCol.Button);
    RenderNavHighlight(bb, id);
    RenderFrame(bb.Min, bb.Max, col, true, ImClamp(cast(float)ImMin(padding.x, padding.y), 0.0f, style.FrameRounding));
    if (bg_col.w > 0.0f)
...
[
    ...
    { ubyte.sizeof,    "%u",   "%u"    },
    { short.sizeof,            "%d",   "%d"    },  // ImGuiDataType_S16
    { ushort.sizeof,   "%u",   "%u"    },
    { int.sizeof,              "%d",   "%d"    },  // ImGuiDataType_S32
    { uint.sizeof,     "%u",   "%u"    },
    { ImS64.sizeof,            "%lld", "%lld"  },  // ImGuiDataType_S64
    { ImU64.sizeof,            "%llu", "%llu"  },
...
bool TempInputScalar(const ImRect/*&*/ bb, ImGuiID id, string label, ImGuiDataType data_type, void* p_data, string format)
{
    ImGuiContext* g = GImGui;
...
bool InputScalar(string label, ImGuiDataType data_type, void* p_data, const void* p_step = NULL, const void* p_step_fast = NULL, string format = NULL, ImGuiInputTextFlags flags = ImGuiInputTextFlags.None)
{
    ...
    flags |= ImGuiInputTextFlags.NoMarkEdited;  // We call MarkItemEdited() ourselve by comparing the actual data rather than the string.
...
bool STB_TEXTEDIT_INSERTCHARS(STB_TEXTEDIT_STRING* obj, int pos, const ImWchar* new_text, int new_text_len)
{
    ...
    if (pos != text_len)
        memmove(text.ptr + pos + new_text_len, text.ptr + pos, cast(size_t)(text_len - pos) * (ImWchar).sizeof);
    memcpy(text.ptr + pos, new_text, cast(size_t)new_text_len * (ImWchar).sizeof);
...
bool InputTextFilterCharacter(uint* p_char, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data)
{
    ...
    if (flags & ImGuiInputTextFlags.CallbackCharFilter)
    {
        ...
        memset(&callback_data, 0, (ImGuiInputTextCallbackData).sizeof);
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id)
    {
        ...
        else if (is_cut || is_copy)
        {
            ...
            if (io.SetClipboardTextFn)
            {
                ...
                char[] clipboard_data = IM_ALLOC!char(clipboard_data_len * (char).sizeof);
...
bool ColorPicker4(string label, float[/*4*/] col, ImGuiColorEditFlags flags = ImGuiColorEditFlags.None, const float* ref_col = NULL)
{
    ...
    else if (flags & ImGuiColorEditFlags.PickerHueBar)
    {
        ...
        sv_cursor_pos.x = ImClamp(IM_ROUND(picker_pos.x + ImSaturate(S)     * sv_picker_size + 0.5f), picker_pos.x + 2, picker_pos.x + sv_picker_size - 2); // Sneakily prevent the circle to stick out too much
        sv_cursor_pos.y = ImClamp(IM_ROUND(picker_pos.y + ImSaturate(1 - V) * sv_picker_size + 0.5f), picker_pos.y + 2, picker_pos.y + sv_picker_size - 2);

        // Render Hue Bar
        for (int i = 0; i < 6; ++i)
            draw_list.AddRectFilledMultiColor(ImVec2(bar0_pos_x, picker_pos.y + i * (sv_picker_size / 6)), ImVec2(bar0_pos_x + bars_width, picker_pos.y + (i + 1) * (sv_picker_size / 6)), col_hues[i], col_hues[i], col_hues[i + 1], col_hues[i + 1]);
        float bar0_line_y = IM_ROUND(picker_pos.y + H * sv_picker_size + 0.5f);
    ...
    if (value_changed && memcmp(backup_initial_col.ptr, col.ptr, components * (float).sizeof) == 0)
        value_changed = false;
    if (value_changed)
        MarkItemEdited(window.DC.LastItemId);
...
bool ColorButton(string desc_id, const ImVec4/*&*/ col, ImGuiColorEditFlags flags = ImGuiColorEditFlags.None, ImVec2 size = ImVec2(0,0))
{
    ...
    {
        if (flags & ImGuiColorEditFlags.NoAlpha)
            SetDragDropPayload(IMGUI_PAYLOAD_TYPE_COLOR_3F, &col_rgb, (float).sizeof * 3, ImGuiCond.Once);
        else
            SetDragDropPayload(IMGUI_PAYLOAD_TYPE_COLOR_4F, &col_rgb, (float).sizeof * 4, ImGuiCond.Once);
...
bool TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, string label)
{
    ...
    if (is_mouse_x_over_arrow && (flags & ImGuiTreeNodeFlags.OpenOnArrow))
        ...
        button_flags |= ImGuiButtonFlags.PressedOnDoubleClick;
    else
...
static float Plot_ArrayGetter(void* data, int idx)
{
    ...
    const float v = plot_data.Values[cast(size_t)idx * plot_data.Stride / float.sizeof];
    return v;
}
...
static if (false) {
    ...
    if (g.HoveredId == id && !held && g.HoveredIdNotActiveTimer > 0.50f && IsItemHovered())
        if (!(tab_bar.Flags & ImGuiTabBarFlags.NoTooltip))
...
bool TabItemLabelAndCloseButton(ImDrawList* draw_list, const ImRect/*&*/ bb, ImGuiTabItemFlags flags, ImVec2 frame_padding, string label, ImGuiID tab_id, ImGuiID close_button_id)
{
    ImGuiContext* g = GImGui;
    ImVec2 label_size = CalcTextSize(label, true);
    if (bb.GetWidth() <= 1.0f)
