...
bool ButtonBehavior(const ImRect/*&*/ bb, ImGuiID id, bool* out_hovered, bool* out_held, ImGuiButtonFlags flags = ImGuiButtonFlags.None)
{
    ...
    if (flags & ImGuiButtonFlags.AllowOverlap)
        item_flags |= ImGuiItemFlags.AllowOverlap;
    if (flags & ImGuiButtonFlags.Repeat)
        item_flags |= ImGuiItemFlags.ButtonRepeat;
...
version (IMGUI_ENABLE_TEST_ENGINE) {
    ...
    // Mouse handling
    const ImGuiID test_owner_id = (flags & ImGuiButtonFlags.NoTestKeyOwner) ? ImGuiKeyOwner.Any : id;
    if (hovered)
    {
        // Poll mouse buttons
        ...
        for (int button = 0; button < 3; button++)
            if (flags & (ImGuiButtonFlags.MouseButtonLeft << button)) // Handle ImGuiButtonFlags_MouseButtonRight and ImGuiButtonFlags_MouseButtonMiddle here.
            {
                if (IsMouseClicked(cast(ImGuiMouseButton)button, test_owner_id) && mouse_button_clicked == -1) { mouse_button_clicked = button; }
        ...
        if (!(flags & ImGuiButtonFlags.NoKeyModifiers) || (!g.IO.KeyCtrl && !g.IO.KeyShift && !g.IO.KeyAlt))
        {
            ...
            {
                if (!(flags & ImGuiButtonFlags.NoSetKeyOwner))
                    SetKeyOwner(MouseButtonToKey(cast(ImGuiMouseButton)mouse_button_clicked), id);
            ...
            if (flags & ImGuiButtonFlags.PressedOnRelease)
            {
                if (mouse_button_released != ImGuiMouseButton.None)
                {
                    const bool has_repeated_at_least_once = (item_flags & ImGuiItemFlags.ButtonRepeat) && g.IO.MouseDownDurationPrev[mouse_button_released] >= g.IO.KeyRepeatDelay; // Repeat mode trumps on release behavior
            ...
            if (g.ActiveId == id && (item_flags & ImGuiItemFlags.ButtonRepeat))
                if (g.IO.MouseDownDuration[g.ActiveIdMouseButton] > 0.0f && IsMouseClicked(g.ActiveIdMouseButton, test_owner_id, ImGuiInputFlags.Repeat))
                    pressed = true;
        }
    ...
    if (g.NavActivateDownId == id)
    {
        ...
        if (!nav_activated_by_inputs && (item_flags & ImGuiItemFlags.ButtonRepeat))
        {
    ...
    if (g.ActiveId == id)
    {
        if (g.ActiveIdSource == ImGuiInputSource.Mouse)
        {
            ...
            else if (IsMouseDown(cast(ImGuiMouseButton)mouse_button, test_owner_id))
            {
                held = true;
            }
            else
            {
                ...
                if ((release_in || release_anywhere) && !g.DragDropActive)
                {
                    ...
                    bool is_repeating_already = (item_flags & ImGuiItemFlags.ButtonRepeat) && g.IO.MouseDownDurationPrev[mouse_button] >= g.IO.KeyRepeatDelay; // Repeat mode trumps <on release>
                    bool is_button_avail_or_owned = TestKeyOwner(MouseButtonToKey(cast(ImGuiMouseButton)mouse_button), test_owner_id);
        ...
        else if (g.ActiveIdSource == ImGuiInputSource.Keyboard || g.ActiveIdSource == ImGuiInputSource.Gamepad)
        {
...
{
    return window.GetID(axis == ImGuiAxis.X ? "#SCROLLX" : "#SCROLLY");
}
...
bool ScrollbarEx(const ImRect/*&*/ bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    bool held = false;
    bool hovered = false;
    ItemAdd(bb_frame, id, NULL, ImGuiItemFlags.NoNav);
...
bool ImageButtonEx(ImGuiID id, ImTextureID texture_id, const ImVec2/*&*/ size, const ImVec2/*&*/ uv0, const ImVec2/*&*/ uv1, const ImVec4/*&*/ bg_col, const ImVec4/*&*/ tint_col, ImGuiButtonFlags flags = ImGuiButtonFlags.None)
{
    ...
    const ImRect bb = ImRect(window.DC.CursorPos, window.DC.CursorPos + size + padding * 2.0f);
    ItemSize(bb);
    if (!ItemAdd(bb, id))
        return false;
...
bool ImageButton(string str_id, ImTextureID user_texture_id, const ImVec2/*&*/ size, const ImVec2/*&*/ uv0, const ImVec2/*&*/ uv1, const ImVec4/*&*/ bg_col, const ImVec4/*&*/ tint_col)
{
    ImGuiContext* g = GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    if (window.SkipItems)
...
bool ImageButton(ImTextureID user_texture_id, const ImVec2/*&*/ size, const ImVec2/*&*/ uv0 = ImVec2(0, 0),  const ImVec2/*&*/ uv1 = ImVec2(1,1), int frame_padding = -1, const ImVec4/*&*/ bg_col = ImVec4(0,0,0,0), const ImVec4/*&*/ tint_col = ImVec4(1,1,1,1))
{
    ...
    if (frame_padding >= 0)
        PushStyleVar(ImGuiStyleVar.FramePadding, ImVec2(cast(float)frame_padding, cast(float)frame_padding));
...
bool CheckboxFlagsT(T)(string label, T* flags, T flags_value)
{
    ...
    {
        ImGuiContext* g = GImGui;
        g.NextItemData.ItemFlags |= ImGuiItemFlags.MixedValue;
...
bool SplitterBehavior(const ImRect/*&*/ bb, ImGuiID id, ImGuiAxis axis, float* size1, float* size2, float min_size1, float min_size2, float hover_extend = 0.0f, float hover_visibility_delay = 0.0f, ImU32 bg_col = 0)
{
    ...
    if (!ItemAdd(bb, id, NULL, ImGuiItemFlags.NoNav))
        return false;
    ...
    ImGuiButtonFlags button_flags = ImGuiButtonFlags.FlattenChildren;
static if (!IMGUI_DISABLE_OBSOLETE_FUNCTIONS) {
    button_flags |= ImGuiButtonFlags.AllowOverlap;
}
...
bool BeginComboPreview()
{
    ...
    if (window.SkipItems || !(g.LastItemData.StatusFlags & ImGuiItemStatusFlags.Visible))
        return false;
...
bool DataTypeApplyFromText(string buf, ImGuiDataType data_type, void* p_data, string format)
{
    ...
    int v32 = 0;
    if (sscanf(buf, format, type_info.Size >= 4 ? p_data : cast(void* )&v32) < 1)
        return false;
    if (type_info.Size < 4)
    {
        if (data_type == ImGuiDataType.S8)
...
TYPE RoundScalarWithFormatT(TYPE)(string format, ImGuiDataType data_type, TYPE v)
{
    IM_UNUSED(data_type);
    IM_ASSERT(data_type == ImGuiDataType.Float || data_type == ImGuiDataType.Double);
    size_t fmt_start = ImParseFormatFindStart(format);
    if (format.length <= fmt_start+1 || format[fmt_start] != '%' || (format.length >= fmt_start+2 && format[fmt_start+1] == '%')) // Don't apply if the value is not visible in the format string
        return v;

    // Sanitize format
    char[32] fmt_sanitized;
    format = ImParseFormatSanitizeForPrinting(format[fmt_start..$], fmt_sanitized);
...
bool DragBehaviorT(TYPE, SIGNEDTYPE, FLOATTYPE)(ImGuiDataType data_type, TYPE* v, float v_speed, TYPE v_min, TYPE v_max, string format, ImGuiSliderFlags flags)
{
    ...
    else if (g.ActiveIdSource == ImGuiInputSource.Keyboard || g.ActiveIdSource == ImGuiInputSource.Gamepad)
    {
        ...
        const bool tweak_slow = IsKeyDown((g.NavInputSource == ImGuiInputSource.Gamepad) ? ImGuiKey.NavGamepadTweakSlow : ImGuiKey.NavKeyboardTweakSlow);
        const bool tweak_fast = IsKeyDown((g.NavInputSource == ImGuiInputSource.Gamepad) ? ImGuiKey.NavGamepadTweakFast : ImGuiKey.NavKeyboardTweakFast);
...
bool DragBehavior(ImGuiID id, ImGuiDataType data_type, void* p_v, float v_speed, const void* p_min, const void* p_max, string format, ImGuiSliderFlags flags)
{
    ...
    if (g.ActiveId == id)
    {
        ...
        if (g.ActiveIdSource == ImGuiInputSource.Mouse && !g.IO.MouseDown[0])
            ClearActiveID();
        else if ((g.ActiveIdSource == ImGuiInputSource.Keyboard || g.ActiveIdSource == ImGuiInputSource.Gamepad) && g.NavActivatePressedId == id && !g.ActiveIdIsJustActivated)
            ClearActiveID();
    }
    if (g.ActiveId != id)
        return false;
...
bool SliderBehaviorT(TYPE, SIGNEDTYPE, FLOATTYPE)(const /*ref*/ ImRect bb, ImGuiID id, ImGuiDataType data_type, TYPE* v, TYPE v_min, TYPE v_max, string format, ImGuiSliderFlags flags, ImRect* out_grab_bb)
{
    ...
    if (g.ActiveId == id)
    {
        ...
        else if (g.ActiveIdSource == ImGuiInputSource.Keyboard || g.ActiveIdSource == ImGuiInputSource.Gamepad)
        {
            ...
            float input_delta = (axis == ImGuiAxis.X) ? GetNavTweakPressedAmount(axis) : -GetNavTweakPressedAmount(axis);
            if (input_delta != 0.0f)
            {
                const bool tweak_slow = IsKeyDown((g.NavInputSource == ImGuiInputSource.Gamepad) ? ImGuiKey.NavGamepadTweakSlow : ImGuiKey.NavKeyboardTweakSlow);
                const bool tweak_fast = IsKeyDown((g.NavInputSource == ImGuiInputSource.Gamepad) ? ImGuiKey.NavGamepadTweakFast : ImGuiKey.NavKeyboardTweakFast);
...
string ImParseFormatSanitizeForPrinting(string fmt_in, char[] fmt_out)
{
    size_t fmt_end = ImParseFormatFindEnd(fmt_in, 0);
    IM_UNUSED(fmt_out.length);
    IM_ASSERT(cast(size_t)(fmt_end + 1) < fmt_out.length); // Format is too long, let us know if this happens to you!
    int j = 0;
    for (int i = 0; i < fmt_end; i++)
    {
        char c = fmt_in[i];
    ...
    return cast(string)fmt_out[0..j];
}
...
string ImParseFormatSanitizeForScanning(string fmt_in, char[] fmt_out)
{
    size_t fmt_end = ImParseFormatFindEnd(fmt_in, 0);
    //string fmt_out_begin = fmt_out;
    IM_UNUSED(fmt_out.length);
    IM_ASSERT(cast(size_t)(fmt_end + 1) < fmt_out.length); // Format is too long, let us know if this happens to you!
    bool has_type = false;
    int j = 0;
...
static pragma(inline, true) ImGuiInputTextFlags InputScalar_DefaultCharsFilter(ImGuiDataType data_type, string format)
{
    if (data_type == ImGuiDataType.Float || data_type == ImGuiDataType.Double)
        return ImGuiInputTextFlags.CharsScientific;
    const char format_last_char = format.length > 0 ? format[format.length - 1] : 0;
    return (format_last_char == 'x' || format_last_char == 'X') ? ImGuiInputTextFlags.CharsHexadecimal : ImGuiInputTextFlags.CharsDecimal;
}
...
bool TempInputScalar(const ImRect/*&*/ bb, ImGuiID id, string label, ImGuiDataType data_type, void* p_data, string format, const (void)* p_clamp_min = NULL, const (void)* p_clamp_max = NULL)
{
    ...
    if (TempInputText(bb, id, label, data_buf, flags))
    {
        ...
        DataTypeApplyFromText(ImCstring(data_buf), data_type, p_data, format);
...
bool InputScalar(string label, ImGuiDataType data_type, void* p_data, const void* p_step = NULL, const void* p_step_fast = NULL, string format = NULL, ImGuiInputTextFlags flags = ImGuiInputTextFlags.None)
{
    ...
    if (g.ActiveId == 0 && (flags & (ImGuiInputTextFlags.CharsDecimal | ImGuiInputTextFlags.CharsHexadecimal | ImGuiInputTextFlags.CharsScientific)) == 0)
        flags |= InputScalar_DefaultCharsFilter(data_type, format);
    flags |= ImGuiInputTextFlags.AutoSelectAll | ImGuiInputTextFlags.NoMarkEdited; // We call MarkItemEdited() ourselves by comparing the actual data rather than the string.
...
{
    IM_ASSERT(!(flags & ImGuiInputTextFlags.Multiline)); // call InputTextMultiline() or  InputTextEx() manually if you need multi-line + hint.
...
static int is_word_boundary_from_right(ImGuiInputTextState* obj, int idx)
{
    ...
    if ((obj.Flags & ImGuiInputTextFlags.Password) || idx <= 0)
        return 0;
...
static bool InputTextFilterCharacter(uint* p_char, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data, ImGuiInputSource input_source)
{
    ...
    {
        bool pass = false;
        pass |= (c == '\n' && (flags & ImGuiInputTextFlags.Multiline)); // Note that an Enter KEY will emit \r and be ignored (we poll for KEY in InputText() code)
    ...
    if (apply_named_filters && (flags & (ImGuiInputTextFlags.CharsDecimal | ImGuiInputTextFlags.CharsHexadecimal | ImGuiInputTextFlags.CharsUppercase | ImGuiInputTextFlags.CharsNoBlank | ImGuiInputTextFlags.CharsScientific)))
    {
        ...
        if (flags & (ImGuiInputTextFlags.CharsDecimal | ImGuiInputTextFlags.CharsScientific | ImGuiInputTextFlags.CharsHexadecimal))
...
static void InputTextReconcileUndoStateAfterUserCallback(ImGuiInputTextState* state, string new_buf_a)
{
    ImGuiContext* g = GImGui;
    const ImWchar* old_buf = state.TextW.Data;
    const int old_length = state.CurLenW;
    const int new_length = ImTextCountCharsFromUtf8(new_buf_a);
    g.TempBuffer.reserve_discard((new_length + 1) * sizeof!(ImWchar));
    ImWchar* new_buf = cast(ImWchar*)cast(void*)g.TempBuffer.Data;
...
bool InputTextEx(string label, string hint, char[] buf, const ImVec2/*&*/ size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback = NULL, void* callback_user_data = NULL)
{
    ...
    if ((init_state && g.ActiveId != id) || init_changed_specs)
    {
        ...
        if (recycle_state && (state.CurLenA != buf_len || (state.TextAIsValid && strcmp(state.TextA.asArray(), buf) != 0)))
            recycle_state = false;

        // Start edition
        string buf_end = NULL;
        state.ID = id;
        state.TextW.resize(cast(int)buf_size + 1);          // wchar count <= UTF-8 count. we use +1 to make sure that .Data is always pointing to at least an empty string.
    ...
    if (g.ActiveId == id)
    {
        ...
        if (is_multiline || (flags & ImGuiInputTextFlags.CallbackHistory))
            g.ActiveIdUsingNavDirMask |= (1 << ImGuiDir.Up) | (1 << ImGuiDir.Down);
        SetKeyOwner(ImGuiKey.Home, id);
        SetKeyOwner(ImGuiKey.End, id);
        if (is_multiline)
        {
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id)
    {
        ...
        const ImGuiInputFlags f_repeat = ImGuiInputFlags.Repeat;
        const bool is_cut   = (Shortcut(ImGuiMod.Shortcut | ImGuiKey.X, id, f_repeat) || Shortcut(ImGuiMod.Shift | ImGuiKey.Delete, id, f_repeat)) && !is_readonly && !is_password && (!is_multiline || state.HasSelection());
        const bool is_copy  = (Shortcut(ImGuiMod.Shortcut | ImGuiKey.C, id) || Shortcut(ImGuiMod.Ctrl | ImGuiKey.Insert, id))  && !is_password && (!is_multiline || state.HasSelection());
        const bool is_paste = (Shortcut(ImGuiMod.Shortcut | ImGuiKey.V, id, f_repeat) || Shortcut(ImGuiMod.Shift | ImGuiKey.Insert, id, f_repeat)) && !is_readonly;
        const bool is_undo  = (Shortcut(ImGuiMod.Shortcut | ImGuiKey.Z, id, f_repeat)) && !is_readonly && is_undoable;
        const bool is_redo =  (Shortcut(ImGuiMod.Shortcut | ImGuiKey.Y, id, f_repeat) || (is_osx && Shortcut(ImGuiMod.Shortcut | ImGuiMod.Shift | ImGuiKey.Z, id, f_repeat))) && !is_readonly && is_undoable;
        const bool is_select_all = Shortcut(ImGuiMod.Shortcut | ImGuiKey.A, id);
        ...
        const bool is_enter_pressed = IsKeyPressed(ImGuiKey.Enter, true) || IsKeyPressed(ImGuiKey.KeypadEnter, true);
        const bool is_gamepad_validate = nav_gamepad_active && (IsKeyPressed(ImGuiKey.NavGamepadActivate, false) || IsKeyPressed(ImGuiKey.NavGamepadInput, false));
        const bool is_cancel = Shortcut(ImGuiKey.Escape, id, f_repeat) || (nav_gamepad_active && Shortcut(ImGuiKey.NavGamepadCancel, id, f_repeat));
        ...
        {
            if (flags & ImGuiInputTextFlags.EscapeClearsAll)
            {
                if (state.CurLenA > 0)
                {
                    revert_edit = true;
    ...
    if (g.ActiveId == id)
    {
        ...
        if (revert_edit && !is_readonly)
        {
            ...
            else if (strcmp(buf, state.InitialTextA.asArray()) != 0)
            {
        ...
        const bool apply_edit_back_to_user_buffer = !revert_edit || (validated && (flags & ImGuiInputTextFlags.EnterReturnsTrue) != 0);
        if (apply_edit_back_to_user_buffer)
        {
            ...
            if ((flags & (ImGuiInputTextFlags.CallbackCompletion | ImGuiInputTextFlags.CallbackHistory | ImGuiInputTextFlags.CallbackEdit | ImGuiInputTextFlags.CallbackAlways)) != 0)
            {
                ...
                if (event_flag)
                {
                    ...
                    if (buf_dirty)
                    {
                        IM_ASSERT((flags & ImGuiInputTextFlags.ReadOnly) == 0);
                        ...
                        InputTextReconcileUndoStateAfterUserCallback(state, cast(string)callback_data.Buf[0..callback_data.BufTextLen]); // FIXME: Move the rest of this block inside function and rename to InputTextReconcileStateAfterUserCallback() ?
    ...
    if (g.InputTextDeactivatedState.ID == id)
    {
        ...
        {
            apply_new_text = cast(string)g.InputTextDeactivatedState.TextA.asArray();
            apply_new_text_length = g.InputTextDeactivatedState.TextA.Size - 1;
            value_changed |= (strcmp(apply_new_text, buf) != 0);
    ...
    if (is_multiline)
    {
        ...
        g.NextItemData.ItemFlags |= ImGuiItemFlags.Inputable | ImGuiItemFlags.NoTabStop;
        EndChild();
    ...
    if (BeginChild("undopoints", ImVec2(0.0f, GetTextLineHeight() * 15), true)) // Visualize undo state
    {
        ...
        for (int n = 0; n < STB_TEXTEDIT_UNDOSTATECOUNT; n++)
        {
            ...
            if (undo_rec_type != ' ' && undo_rec.char_storage != -1)
                ImTextStrToUtf8(buf.ptr, IM_ARRAYSIZE(buf), undo_state.undo_char.ptr + undo_rec.char_storage, undo_state.undo_char.ptr + undo_rec.char_storage + undo_rec.insert_length);
...
bool ColorEdit4(string label, float[/*4*/] col, ImGuiColorEditFlags flags = ImGuiColorEditFlags.None)
{
    ...
    if (label_display_end.length != 0 && !(flags & ImGuiColorEditFlags.NoLabel))
    {
        ...
        window.DC.CursorPos.x = pos.x + ((flags & ImGuiColorEditFlags.NoInputs) ? w_button : w_full + style.ItemInnerSpacing.x);
        TextEx(label_display_end);
    }
...
bool ColorButton(string desc_id, const ImVec4/*&*/ col, ImGuiColorEditFlags flags = ImGuiColorEditFlags.None, const ImVec2/*&*/ size_arg = ImVec2(0, 0))
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window.SkipItems)
        return false;
    ...
    const float default_size = GetFrameHeight();
    const ImVec2 size = ImVec2(size_arg.x == 0.0f ? default_size : size_arg.x, size_arg.y == 0.0f ? default_size : size_arg.y);
    const ImRect bb = ImRect(window.DC.CursorPos, window.DC.CursorPos + size);
    ...
    // Tooltip
    if (!(flags & ImGuiColorEditFlags.NoTooltip) && hovered && IsItemHovered(ImGuiHoveredFlags.ForTooltip))
...
bool TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, string label)
{
    ...
    if ((flags & ImGuiTreeNodeFlags.AllowOverlap) || (g.LastItemData.InFlags & ImGuiItemFlags.AllowOverlap))
        button_flags |= ImGuiButtonFlags.AllowOverlap;
    if (!is_leaf)
    ...
    if (display_frame)
    {
        ...
        if (flags & ImGuiTreeNodeFlags.Bullet)
            ...
            RenderArrow(window.DrawList, ImVec2(text_pos.x - text_offset_x + padding.x, text_pos.y), text_col, is_open ? ((flags & ImGuiTreeNodeFlags.UpsideDownArrow) ? ImGuiDir.Up : ImGuiDir.Down) : ImGuiDir.Right, 1.0f);
    ...
    else
    {
        ...
        if (flags & ImGuiTreeNodeFlags.Bullet)
            ...
            RenderArrow(window.DrawList, ImVec2(text_pos.x - text_offset_x + padding.x, text_pos.y + g.FontSize * 0.15f), text_col, is_open ? ((flags & ImGuiTreeNodeFlags.UpsideDownArrow) ? ImGuiDir.Up : ImGuiDir.Down) : ImGuiDir.Right, 0.70f);
        if (g.LogEnabled)
            LogSetNextTextDecoration(">", NULL);
...
bool CollapsingHeader(string label, bool* p_visible, ImGuiTreeNodeFlags flags = ImGuiTreeNodeFlags.None)
{
    ...
    if (p_visible)
        flags |= ImGuiTreeNodeFlags.AllowOverlap | ImGuiTreeNodeFlags.ClipLabelForTrailingButton;
...
bool Selectable(string label, bool selected = false, ImGuiSelectableFlags flags = ImGuiSelectableFlags.None, const ImVec2/*&*/ size_arg = ImVec2(0, 0))
{
    ...
    if (flags & ImGuiSelectableFlags.NoSetKeyOwner)     { button_flags |= ImGuiButtonFlags.NoSetKeyOwner; }
    ...
    if ((flags & ImGuiSelectableFlags.AllowOverlap) || (g.LastItemData.InFlags & ImGuiItemFlags.AllowOverlap)) { button_flags |= ImGuiButtonFlags.AllowOverlap; }
    ...
    if ((flags & ImGuiSelectableFlags.SelectOnNav) && g.NavJustMovedToId != 0 && g.NavJustMovedToFocusScopeId == g.CurrentFocusScopeId)
...
bool BeginMenuEx(string label, string icon, bool enabled)
{
    ...
    ImGuiWindowFlags window_flags = ImGuiWindowFlags.ChildMenu | ImGuiWindowFlags.AlwaysAutoResize | ImGuiWindowFlags.NoMove | ImGuiWindowFlags.NoTitleBar | ImGuiWindowFlags.NoSavedSettings | ImGuiWindowFlags.NoNavFocus;
    if (window.Flags & ImGuiWindowFlags.ChildMenu)
        window_flags |= ImGuiWindowFlags.ChildWindow;
    ...
    const ImGuiSelectableFlags selectable_flags = ImGuiSelectableFlags.NoHoldingActiveID | ImGuiSelectableFlags.NoSetKeyOwner | ImGuiSelectableFlags.SelectOnClick | ImGuiSelectableFlags.DontClosePopups;
    ...
    if (menu_is_open)
    {
        ...
        SetNextWindowPos(popup_pos, ImGuiCond.Always);                  // Note: misleading: the value will serve as reference for FindBestWindowPosForPopup(), not actual pos.
...
void EndMenu()
{
    ...
    ImGuiContext* g = GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    IM_ASSERT(window.Flags & ImGuiWindowFlags.Popup);  // Mismatched BeginMenu()/EndMenu() calls
    ImGuiWindow* parent_window = window.ParentWindow;  // Should always be != NULL is we passed assert.
    if (window.BeginCount == window.BeginCountPreviousFrame)
        if (g.NavMoveDir == ImGuiDir.Left && NavMoveRequestButNoResultYet())
            if (g.NavWindow && (g.NavWindow.RootWindowForNav == window) && parent_window.DC.LayoutType == ImGuiLayoutType.Vertical)
            {
...
bool MenuItemEx(string label, string icon, string shortcut = NULL, bool selected = false, bool enabled = true)
{
    ...
    // We use ImGuiSelectableFlags_NoSetKeyOwner to allow down on one menu item, move, up on another.
    const ImGuiSelectableFlags selectable_flags = ImGuiSelectableFlags.SelectOnRelease | ImGuiSelectableFlags.NoSetKeyOwner | ImGuiSelectableFlags.SetNavIdOnHover;
    const ImGuiMenuColumns* offsets = &window.DC.MenuColumns;
    if (window.DC.LayoutType == ImGuiLayoutType.Horizontal)
    {
    ...
    else
    {
        ...
        // (In a typical menu window where all items are BeginMenu() or MenuItem() calls, extra_w will always be 0.0f.
        //  Only when they are other items sticking out we're going to add spacing, yet only register minimum width into the layout system.
        float icon_w = (icon.length > 0) ? CalcTextSize(icon).x : 0.0f;
        float shortcut_w = (shortcut.length > 0) ? CalcTextSize(shortcut).x : 0.0f;
        float checkmark_w = IM_FLOOR(g.FontSize * 1.20f);
...
namespace ImGui
{
    ...
    static ImU32            TabBarCalcTabID(ImGuiTabBar* tab_bar, string label, ImGuiWindow* docked_window);
...
static void TabBarLayout(ImGuiTabBar* tab_bar)
{
    ...
    if (width_excess >= 1.0f && ((tab_bar.Flags & ImGuiTabBarFlags.FittingPolicyResizeDown) || !central_section_is_visible))
    {
    ...
    else if ((tab_bar.Flags & ImGuiTabBarFlags.FittingPolicyScroll) && IsMouseHoveringRect(tab_bar.BarRect.Min, tab_bar.BarRect.Max, true) && IsWindowContentHoverable(g.CurrentWindow))
    {
        const float wheel = g.IO.MouseWheelRequestAxisSwap ? g.IO.MouseWheel : g.IO.MouseWheelH;
        const ImGuiKey wheel_key = g.IO.MouseWheelRequestAxisSwap ? ImGuiKey.MouseWheelY : ImGuiKey.MouseWheelX;
        if (TestKeyOwner(wheel_key, tab_bar.ID) && wheel != 0.0f)
        {
            const float scroll_step = wheel * TabBarCalcScrollableWidth(tab_bar, sections.ptr) / 3.0f;
...
bool    TabItemButton(string label, ImGuiTabItemFlags flags)
{
    ...
    return TabItemEx(tab_bar, label, NULL, flags | ImGuiTabItemFlags.Button | ImGuiTabItemFlags.NoReorder, NULL);
}
...
bool    TabItemEx(ImGuiTabBar* tab_bar, string label, bool* p_open, ImGuiTabItemFlags flags, ImGuiWindow* docked_window)
{
    ...
    if (p_open && !*p_open)
    {
        ItemAdd(ImRect(), id, NULL, ImGuiItemFlags.NoNav);
        return false;
    }
    ...
    ImVec2 size = TabItemCalcSize(label, (p_open != NULL) || (flags & ImGuiTabItemFlags.UnsavedDocument));
    ...
    const bool tab_just_unsaved = (flags & ImGuiTabItemFlags.UnsavedDocument) && !(tab.Flags & ImGuiTabItemFlags.UnsavedDocument);
    ...
    ImGuiButtonFlags button_flags = ((is_tab_button ? ImGuiButtonFlags.PressedOnClickRelease : ImGuiButtonFlags.PressedOnClick) | ImGuiButtonFlags.AllowOverlap);
    if (g.DragDropActive)
        button_flags |= ImGuiButtonFlags.PressedOnDragDropHold;
    bool hovered, held;
    bool pressed = ButtonBehavior(bb, id, &hovered, &held, button_flags);
...
static if (false) {
    ...
    bool just_closed;
    bool text_clipped;
    TabItemLabelAndCloseButton(display_draw_list, bb, tab_just_unsaved ? (flags & ~ImGuiTabItemFlags.UnsavedDocument) : flags, tab_bar.FramePadding, label, id, close_button_id, tab_contents_visible, &just_closed, &text_clipped);
    ...
    if (text_clipped && g.HoveredId == id && !held)
        if (!(tab_bar.Flags & ImGuiTabBarFlags.NoTooltip) && !(tab.Flags & ImGuiTabItemFlags.NoTooltip))
            SetItemTooltip("%.*s", FindRenderedTextEnd(label));
...
ImVec2 TabItemCalcSize(ImGuiWindow*)
{
