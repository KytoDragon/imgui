...
{
    PushStyleColor(ImGuiCol.Text, GImGui.Style.Colors[ImGuiCol.TextDisabled]);
    TextV(fmt, args);
    PopStyleColor();
}
...
void LabelTextV(string label, string fmt, va_list args)
{
    ...
    const ImRect value_bb = ImRect(window.DC.CursorPos, window.DC.CursorPos + ImVec2(w, label_size.y + style.FramePadding.y*2));
    const ImRect total_bb = ImRect(window.DC.CursorPos, window.DC.CursorPos + ImVec2(w + (label_size.x > 0.0f ? style.ItemInnerSpacing.x : 0.0f), style.FramePadding.y*2) + label_size);
    ...
    RenderTextClipped(value_bb.Min, value_bb.Max, cast(string)g.TempBuffer[0..value_text_end], NULL, ImVec2(0.0f,0.5f));
...
void BulletTextV(string fmt, va_list args)
{
    ...
    int text_end = ImFormatStringV(g.TempBuffer, fmt, args);
    const ImVec2 label_size = CalcTextSize(cast(string)g.TempBuffer[0..text_end], false);
    ...
    RenderText(bb.Min + ImVec2(g.FontSize + style.FramePadding.x * 2, 0.0f), cast(string)g.TempBuffer[0..text_end], false);
}
...
bool ButtonBehavior(const ImRect/*&*/ bb, ImGuiID id, bool* out_hovered, bool* out_held, ImGuiButtonFlags flags = ImGuiButtonFlags.None)
{
    ...
    if (flags & ImGuiButtonFlags.Disabled)
    {
    ...
    const bool flatten_hovered_children = (flags & ImGuiButtonFlags.FlattenChildren) && g.HoveredRootWindow == window;
...
bool ButtonEx(string label, const ImVec2/*&*/ size_arg = ImVec2(0, 0), ImGuiButtonFlags flags = ImGuiButtonFlags.None)
{
    ...
    if (window.DC.ItemFlags & ImGuiItemFlags.ButtonRepeat)
...
bool InvisibleButton(string str_id, const ImVec2/*&*/ size_arg)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window.SkipItems)
        return false;
...
bool CloseButton(ImGuiID id, const ImVec2/*&*/ pos)//, float size)
{
    ImGuiContext* g = GImGui;
...
void Scrollbar(ImGuiAxis axis)
{
    ...
    ImDrawCornerFlags rounding_corners = ImDrawCornerFlags.None;
    if (axis == ImGuiAxis.X)
    {
        rounding_corners |= ImDrawCornerFlags.BotLeft;
        if (!window.ScrollbarY)
            rounding_corners |= ImDrawCornerFlags.BotRight;
    }
    else
    {
        if ((window.Flags & ImGuiWindowFlags.NoTitleBar) && !(window.Flags & ImGuiWindowFlags.MenuBar))
            rounding_corners |= ImDrawCornerFlags.TopRight;
...
bool ScrollbarEx(const ImRect/*&*/ bb_frame, ImGuiID id, ImGuiAxis axis, float* p_scroll_v, float size_avail_v, float size_contents_v, ImDrawCornerFlags rounding_corners)
{
    ImGuiContext* g = GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    if (window.SkipItems)
...
bool ImageButtonEx(ImGuiID id, ImTextureID user_texture_id, const ImVec2/*&*/ size, const ImVec2/*&*/ uv0, const ImVec2/*&*/ uv1, const ImVec2/*&*/ padding, const ImVec4/*&*/ bg_col, const ImVec4/*&*/ tint_col)
{
    ImGuiContext* g = GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    if (window.SkipItems)
...
bool Checkbox(string label, bool* v)
{
    ...
    if (window.DC.ItemFlags & ImGuiItemFlags.MixedValue)
    {
    ...
    IMGUI_TEST_ENGINE_ITEM_INFO(id, label, cast(ImGuiItemStatusFlags)(window.DC.ItemFlags | ImGuiItemStatusFlags.Checkable | (*v ? ImGuiItemStatusFlags.Checked : ImGuiItemStatusFlags.None)));
    return pressed;
}
...
void SeparatorEx(ImGuiSeparatorFlags flags)
{
    ...
    else if (flags & ImGuiSeparatorFlags.Horizontal)
    {
        ...
        ImGuiColumns* columns = (flags & ImGuiSeparatorFlags.SpanAllColumns) ? window.DC.CurrentColumns : NULL;
        if (columns)
...
bool SplitterBehavior(const ImRect/*&*/ bb, ImGuiID id, ImGuiAxis axis, float* size1, float* size2, float min_size1, float min_size2, float hover_extend = 0.0f, float hover_visibility_delay = 0.0f)
{
    ...
    window.DC.ItemFlags |= ImGuiItemFlags.NoNav | ImGuiItemFlags.NoNavDefaultFocus;
...
int ShrinkWidthItemComparer(ImGuiShrinkWidthItem* a, ImGuiShrinkWidthItem* b)
{
...
bool BeginCombo(string label, string preview_value, ImGuiComboFlags flags = ImGuiComboFlags.None)
{
    ...
    ImGuiContext* g = GImGui;
    bool has_window_size_constraint = (g.NextWindowData.Flags & ImGuiNextWindowDataFlags.HasSizeConstraint) != 0;
    ...
    const float w = (flags & ImGuiComboFlags.NoPreview) ? arrow_size : expected_w;
    ...
    bool popup_open = IsPopupOpen(id, ImGuiPopupFlags.None);
    ...
    if (!(flags & ImGuiComboFlags.NoPreview))
        window.DrawList.AddRectFilled(frame_bb.Min, ImVec2(value_x2, frame_bb.Max.y), frame_col, style.FrameRounding, (flags & ImGuiComboFlags.NoArrowButton) ? ImDrawCornerFlags.All : ImDrawCornerFlags.Left);
    if (!(flags & ImGuiComboFlags.NoArrowButton))
    {
        ImU32 bg_col = GetColorU32((popup_open || hovered) ? ImGuiCol.ButtonHovered : ImGuiCol.Button);
        ImU32 text_col = GetColorU32(ImGuiCol.Text);
        window.DrawList.AddRectFilled(ImVec2(value_x2, frame_bb.Min.y), frame_bb.Max, bg_col, style.FrameRounding, (w <= arrow_size) ? ImDrawCornerFlags.All : ImDrawCornerFlags.Right);
        if (value_x2 + arrow_size - style.FramePadding.x <= frame_bb.Max.x)
            RenderArrow(window.DrawList, ImVec2(value_x2 + style.FramePadding.y, frame_bb.Min.y + style.FramePadding.y), text_col, ImGuiDir.Down, 1.0f);
    }
    ...
    if ((pressed || g.NavActivateId == id) && !popup_open)
    {
        ...
        OpenPopupEx(id, ImGuiPopupFlags.None);
        popup_open = true;
    }
    ...
    if (ImGuiWindow* popup_window = FindWindowByName(cast(string)name[0..length]))
        if (popup_window.WasActive)
        {
            ...
            if (flags & ImGuiComboFlags.PopupAlignLeft)
                popup_window.AutoPosLastDirection = ImGuiDir.Left;
            ImRect r_outer = GetWindowAllowedExtentRect(popup_window);
            ImVec2 pos = FindBestWindowPosForPopupEx(frame_bb.GetBL(), size_expected, &popup_window.AutoPosLastDirection, r_outer, frame_bb, ImGuiPopupPositionPolicy.ComboBox);
    ...
    PushStyleVar(ImGuiStyleVar.WindowPadding, ImVec2(style.FramePadding.x, style.WindowPadding.y));
...
[
    { sizeof!(byte),             "%d",   "%d"    },  // ImGuiDataType_S8
    { sizeof!(ubyte),    "%u",   "%u"    },
    ...
    { sizeof!(double),           "%lf",   "%lf"  },  // ImGuiDataType_Double
];
...
void DataTypeApplyOp(ImGuiDataType data_type, int op, void* output, void* arg1, const void* arg2)
{
    IM_ASSERT(op == '+' || op == '-');
    switch (data_type)
    {
...
bool ClampBehaviorT(T)(T* v, T v_min, T v_max)
{
...
bool DataTypeClamp(ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max)
{
    switch (data_type)
    {
    case ImGuiDataType.S8:     return ClampBehaviorT!(ImS8  )(cast(ImS8*  )p_data, *cast(const ImS8*  )p_min, *cast(const ImS8*  )p_max);
    case ImGuiDataType.U8:     return ClampBehaviorT!(ImU8  )(cast(ImU8*  )p_data, *cast(const ImU8*  )p_min, *cast(const ImU8*  )p_max);
    case ImGuiDataType.S16:    return ClampBehaviorT!(ImS16 )(cast(ImS16* )p_data, *cast(const ImS16* )p_min, *cast(const ImS16* )p_max);
    case ImGuiDataType.U16:    return ClampBehaviorT!(ImU16 )(cast(ImU16* )p_data, *cast(const ImU16* )p_min, *cast(const ImU16* )p_max);
    case ImGuiDataType.S32:    return ClampBehaviorT!(ImS32 )(cast(ImS32* )p_data, *cast(const ImS32* )p_min, *cast(const ImS32* )p_max);
    case ImGuiDataType.U32:    return ClampBehaviorT!(ImU32 )(cast(ImU32* )p_data, *cast(const ImU32* )p_min, *cast(const ImU32* )p_max);
    case ImGuiDataType.S64:    return ClampBehaviorT!(ImS64 )(cast(ImS64* )p_data, *cast(const ImS64* )p_min, *cast(const ImS64* )p_max);
    case ImGuiDataType.U64:    return ClampBehaviorT!(ImU64 )(cast(ImU64* )p_data, *cast(const ImU64* )p_min, *cast(const ImU64* )p_max);
    case ImGuiDataType.Float:  return ClampBehaviorT!(float )(cast(float* )p_data, *cast(const float* )p_min, *cast(const float* )p_max);
    case ImGuiDataType.Double: return ClampBehaviorT!(double)(cast(double*)p_data, *cast(const double*)p_min, *cast(const double*)p_max);
    case ImGuiDataType.COUNT:  break;
    default: break;
    }
    IM_ASSERT(0);
...
bool DragBehaviorT(TYPE, SIGNEDTYPE, FLOATTYPE)(ImGuiDataType data_type, TYPE* v, float v_speed, TYPE v_min, TYPE v_max, string format, float power, ImGuiDragFlags flags)
{
    ImGuiContext* g = GImGui;
    const ImGuiAxis axis = (flags & ImGuiDragFlags.Vertical) ? ImGuiAxis.Y : ImGuiAxis.X;
    const bool is_decimal = (data_type == ImGuiDataType.Float) || (data_type == ImGuiDataType.Double);
    ...
    if (g.ActiveIdSource == ImGuiInputSource.Mouse && IsMousePosValid() && g.IO.MouseDragMaxDistanceSqr[0] > 1.0f*1.0f)
    {
    ...
    if (is_power)
    {
        ...
        FLOATTYPE v_old_norm_curved = ImPow(cast(FLOATTYPE)(v_cur - v_min) / cast(FLOATTYPE)(v_max - v_min), cast(FLOATTYPE)1.0f / power);
        FLOATTYPE v_new_norm_curved = v_old_norm_curved + (g.DragCurrentAccum / (v_max - v_min));
        v_cur = v_min + cast(SIGNEDTYPE)ImPow(ImSaturate(cast(float)v_new_norm_curved), power) * (v_max - v_min);
    ...
    if (is_power)
    {
        FLOATTYPE v_cur_norm_curved = ImPow(cast(FLOATTYPE)(v_cur - v_min) / cast(FLOATTYPE)(v_max - v_min), cast(FLOATTYPE)1.0f / power);
        g.DragCurrentAccum -= cast(float)(v_cur_norm_curved - v_old_ref_for_accum_remainder);
    }
    else
    {
...
bool DragBehavior(ImGuiID id, ImGuiDataType data_type, void* p_v, float v_speed, const void* p_min, const void* p_max, string format, float power, ImGuiDragFlags flags)
{
    ImGuiContext* g = GImGui;
    if (g.ActiveId == id)
    {
    ...
    switch (data_type)
    {
    case ImGuiDataType.S8:     { ImS32 v32 = cast(ImS32)*cast(ImS8*)p_v;  bool r = DragBehaviorT!(ImS32, ImS32, float)(ImGuiDataType.S32, &v32, v_speed, p_min ? *cast(const ImS8*) p_min : IM_S8_MIN,  p_max ? *cast(const ImS8*)p_max  : IM_S8_MAX,  format, power, flags); if (r) *cast(ImS8*)p_v = cast(ImS8)v32; return r; }
    case ImGuiDataType.U8:     { ImU32 v32 = cast(ImU32)*cast(ImU8*)p_v;  bool r = DragBehaviorT!(ImU32, ImS32, float)(ImGuiDataType.U32, &v32, v_speed, p_min ? *cast(const ImU8*) p_min : IM_U8_MIN,  p_max ? *cast(const ImU8*)p_max  : IM_U8_MAX,  format, power, flags); if (r) *cast(ImU8*)p_v = cast(ImU8)v32; return r; }
    case ImGuiDataType.S16:    { ImS32 v32 = cast(ImS32)*cast(ImS16*)p_v; bool r = DragBehaviorT!(ImS32, ImS32, float)(ImGuiDataType.S32, &v32, v_speed, p_min ? *cast(const ImS16*)p_min : IM_S16_MIN, p_max ? *cast(const ImS16*)p_max : IM_S16_MAX, format, power, flags); if (r) *cast(ImS16*)p_v = cast(ImS16)v32; return r; }
    case ImGuiDataType.U16:    { ImU32 v32 = cast(ImU32)*cast(ImU16*)p_v; bool r = DragBehaviorT!(ImU32, ImS32, float)(ImGuiDataType.U32, &v32, v_speed, p_min ? *cast(const ImU16*)p_min : IM_U16_MIN, p_max ? *cast(const ImU16*)p_max : IM_U16_MAX, format, power, flags); if (r) *cast(ImU16*)p_v = cast(ImU16)v32; return r; }
    case ImGuiDataType.S32:    return DragBehaviorT!(ImS32, ImS32, float )(data_type, cast(ImS32*)p_v,  v_speed, p_min ? *cast(const ImS32* )p_min : IM_S32_MIN, p_max ? *cast(const ImS32* )p_max : IM_S32_MAX, format, power, flags);
    case ImGuiDataType.U32:    return DragBehaviorT!(ImU32, ImS32, float )(data_type, cast(ImU32*)p_v,  v_speed, p_min ? *cast(const ImU32* )p_min : IM_U32_MIN, p_max ? *cast(const ImU32* )p_max : IM_U32_MAX, format, power, flags);
    case ImGuiDataType.S64:    return DragBehaviorT!(ImS64, ImS64, double)(data_type, cast(ImS64*)p_v,  v_speed, p_min ? *cast(const ImS64* )p_min : IM_S64_MIN, p_max ? *cast(const ImS64* )p_max : IM_S64_MAX, format, power, flags);
    case ImGuiDataType.U64:    return DragBehaviorT!(ImU64, ImS64, double)(data_type, cast(ImU64*)p_v,  v_speed, p_min ? *cast(const ImU64* )p_min : IM_U64_MIN, p_max ? *cast(const ImU64* )p_max : IM_U64_MAX, format, power, flags);
    case ImGuiDataType.Float:  return DragBehaviorT!(float, float, float )(data_type, cast(float*)p_v,  v_speed, p_min ? *cast(const float* )p_min : -FLT_MAX,   p_max ? *cast(const float* )p_max : FLT_MAX,    format, power, flags);
    case ImGuiDataType.Double: return DragBehaviorT!(double,double,double)(data_type, cast(double*)p_v, v_speed, p_min ? *cast(const double*)p_min : -DBL_MAX,   p_max ? *cast(const double*)p_max : DBL_MAX,    format, power, flags);
    case ImGuiDataType.COUNT:  break;
    default: break;
    }
    IM_ASSERT(0);
...
bool DragScalar(string label, ImGuiDataType data_type, void* p_data, float v_speed, const void* p_min = NULL, const void* p_max = NULL, string format = NULL, float power = 1.0f)
{
    ...
    // Draw frame
    const ImU32 frame_col = GetColorU32(g.ActiveId == id ? ImGuiCol.FrameBgActive : g.HoveredId == id ? ImGuiCol.FrameBgHovered : ImGuiCol.FrameBg);
    RenderNavHighlight(frame_bb, id);
    RenderFrame(frame_bb.Min, frame_bb.Max, frame_col, true, style.FrameRounding);

    // Drag behavior
    const bool value_changed = DragBehavior(id, data_type, p_data, v_speed, p_min, p_max, format, power, ImGuiDragFlags.None);
    if (value_changed)
        MarkItemEdited(id);
...
bool DragFloat2(string label, ImVec2* v, float v_speed = 1.0f, float v_min = 0.0f, float v_max = 0.0f, string format = "%.3f", float power = 1.0f)
{
...
float SliderCalcRatioFromValueT(TYPE, FLOATTYPE)(ImGuiDataType data_type, TYPE v, TYPE v_min, TYPE v_max, float power, float linear_zero_pos)
{
    if (v_min == v_max)
        return 0.0f;

    const bool is_power = (power != 1.0f) && (data_type == ImGuiDataType.Float || data_type == ImGuiDataType.Double);
    const TYPE v_clamped = (v_min < v_max) ? ImClamp(v, v_min, v_max) : ImClamp(v, v_max, v_min);
    if (is_power)
    {
        if (v_clamped < 0.0f)
        {
            const float f = 1.0f - cast(float)((v_clamped - v_min) / (ImMin(cast(TYPE)0, v_max) - v_min));
            return (1.0f - ImPow(f, 1.0f/power)) * linear_zero_pos;
        }
        else
        {
            const float f = cast(float)((v_clamped - ImMax(cast(TYPE)0, v_min)) / (v_max - ImMax(cast(TYPE)0, v_min)));
            return linear_zero_pos + ImPow(f, 1.0f/power) * (1.0f - linear_zero_pos);
        }
    }

    // Linear slider
    return cast(float)(cast(FLOATTYPE)(v_clamped - v_min) / cast(FLOATTYPE)(v_max - v_min));
}
...
bool SliderBehaviorT(TYPE, SIGNEDTYPE, FLOATTYPE)(const /*ref*/ ImRect bb, ImGuiID id, ImGuiDataType data_type, TYPE* v, TYPE v_min, TYPE v_max, string format, float power, ImGuiSliderFlags flags, ImRect* out_grab_bb)
{
    ImGuiContext* g = GImGui;
    const ImGuiStyle* style = &g.Style;
    ...
    {
        // Different sign
        const FLOATTYPE linear_dist_min_to_0 = ImPow(v_min >= 0 ? cast(FLOATTYPE)v_min : -cast(FLOATTYPE)v_min, cast(FLOATTYPE)1.0f / power);
        const FLOATTYPE linear_dist_max_to_0 = ImPow(v_max >= 0 ? cast(FLOATTYPE)v_max : -cast(FLOATTYPE)v_max, cast(FLOATTYPE)1.0f / power);
        linear_zero_pos = cast(float)(linear_dist_min_to_0 / (linear_dist_min_to_0 + linear_dist_max_to_0));
    }
    else
    {
        // Same sign
    ...
    if (g.ActiveId == id)
    {
        ...
        {
            const ImVec2 delta2 = GetNavInputAmount2d(ImGuiNavDirSourceFlags.Keyboard | ImGuiNavDirSourceFlags.PadDPad, ImGuiInputReadMode.RepeatFast, 0.0f, 0.0f);
            float delta = (axis == ImGuiAxis.X) ? delta2.x : -delta2.y;
            ...
            else if (delta != 0.0f)
            {
                ...
                else
                {
                    if ((v_range >= -100.0f && v_range <= 100.0f) || IsNavInputDown(ImGuiNavInput.TweakSlow))
                        delta = ((delta < 0.0f) ? -1.0f : +1.0f) / cast(float)v_range; // Gamepad/keyboard tweak speeds in integer steps
                    else
                        delta /= 100.0f;
                }
        ...
        if (set_new_value)
        {
            ...
            if (is_power)
            {
                ...
                if (clicked_t < linear_zero_pos)
                {
                    ...
                    a = ImPow(a, power);
                    v_new = ImLerp(ImMin(v_max, cast(TYPE)0), v_min, a);
                }
                else
                {
                    // Positive: rescale to the positive range before powering
                    float a;
                    if (ImFabs(linear_zero_pos - 1.0f) > 1.0e-6f)
                        a = (clicked_t - linear_zero_pos) / (1.0f - linear_zero_pos);
                    else
                        a = clicked_t;
                    a = ImPow(a, power);
                    v_new = ImLerp(ImMax(v_min, cast(TYPE)0), v_max, a);
                }
            }
            else
            {
                ...
                else
                {
                    ...
                    TYPE v_new_off_floor = cast(TYPE)(v_new_off_f);
                    TYPE v_new_off_round = cast(TYPE)(v_new_off_f + cast(FLOATTYPE)0.5);
...
bool SliderBehavior(const ImRect/*&*/ bb, ImGuiID id, ImGuiDataType data_type, void* p_v, const void* p_min, const void* p_max, string format, float power, ImGuiSliderFlags flags, ImRect* out_grab_bb)
{
    switch (data_type)
    {
    case ImGuiDataType.S8:  { ImS32 v32 = cast(ImS32)*cast(ImS8*)p_v;  bool r = SliderBehaviorT!(ImS32, ImS32, float)(bb, id, ImGuiDataType.S32, &v32, *cast(const ImS8*)p_min,  *cast(const ImS8*)p_max,  format, power, flags, out_grab_bb); if (r) *cast(ImS8*)p_v  = cast(ImS8)v32;  return r; }
    case ImGuiDataType.U8:  { ImU32 v32 = cast(ImU32)*cast(ImU8*)p_v;  bool r = SliderBehaviorT!(ImU32, ImS32, float)(bb, id, ImGuiDataType.U32, &v32, *cast(const ImU8*)p_min,  *cast(const ImU8*)p_max,  format, power, flags, out_grab_bb); if (r) *cast(ImU8*)p_v  = cast(ImU8)v32;  return r; }
    case ImGuiDataType.S16: { ImS32 v32 = cast(ImS32)*cast(ImS16*)p_v; bool r = SliderBehaviorT!(ImS32, ImS32, float)(bb, id, ImGuiDataType.S32, &v32, *cast(const ImS16*)p_min, *cast(const ImS16*)p_max, format, power, flags, out_grab_bb); if (r) *cast(ImS16*)p_v = cast(ImS16)v32; return r; }
    case ImGuiDataType.U16: { ImU32 v32 = cast(ImU32)*cast(ImU16*)p_v; bool r = SliderBehaviorT!(ImU32, ImS32, float)(bb, id, ImGuiDataType.U32, &v32, *cast(const ImU16*)p_min, *cast(const ImU16*)p_max, format, power, flags, out_grab_bb); if (r) *cast(ImU16*)p_v = cast(ImU16)v32; return r; }
    case ImGuiDataType.S32:
        IM_ASSERT(*cast(const ImS32*)p_min >= IM_S32_MIN/2 && *cast(const ImS32*)p_max <= IM_S32_MAX/2);
        return SliderBehaviorT!(ImS32, ImS32, float )(bb, id, data_type, cast(ImS32*)p_v,  *cast(const ImS32*)p_min,  *cast(const ImS32*)p_max,  format, power, flags, out_grab_bb);
    case ImGuiDataType.U32:
        IM_ASSERT(*cast(const ImU32*)p_max <= IM_U32_MAX/2);
        return SliderBehaviorT!(ImU32, ImS32, float )(bb, id, data_type, cast(ImU32*)p_v,  *cast(const ImU32*)p_min,  *cast(const ImU32*)p_max,  format, power, flags, out_grab_bb);
    case ImGuiDataType.S64:
        IM_ASSERT(*cast(const ImS64*)p_min >= IM_S64_MIN/2 && *cast(const ImS64*)p_max <= IM_S64_MAX/2);
        return SliderBehaviorT!(ImS64, ImS64, double)(bb, id, data_type, cast(ImS64*)p_v,  *cast(const ImS64*)p_min,  *cast(const ImS64*)p_max,  format, power, flags, out_grab_bb);
    case ImGuiDataType.U64:
        IM_ASSERT(*cast(const ImU64*)p_max <= IM_U64_MAX/2);
        return SliderBehaviorT!(ImU64, ImS64, double)(bb, id, data_type, cast(ImU64*)p_v,  *cast(const ImU64*)p_min,  *cast(const ImU64*)p_max,  format, power, flags, out_grab_bb);
    case ImGuiDataType.Float:
        IM_ASSERT(*cast(const float*)p_min >= -FLT_MAX/2.0f && *cast(const float*)p_max <= FLT_MAX/2.0f);
        return SliderBehaviorT!(float, float, float )(bb, id, data_type, cast(float*)p_v,  *cast(const float*)p_min,  *cast(const float*)p_max,  format, power, flags, out_grab_bb);
    case ImGuiDataType.Double:
        IM_ASSERT(*cast(const double*)p_min >= -DBL_MAX/2.0f && *cast(const double*)p_max <= DBL_MAX/2.0f);
        return SliderBehaviorT!(double,double,double)(bb, id, data_type, cast(double*)p_v, *cast(const double*)p_min, *cast(const double*)p_max, format, power, flags, out_grab_bb);
    case ImGuiDataType.COUNT: break;
    default: break;
    }
    IM_ASSERT(0);
...
bool SliderScalar(string label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, string format = NULL, float power = 1.0f)
{
    ...
    // Slider behavior
    ImRect grab_bb;
    const bool value_changed = SliderBehavior(frame_bb, id, data_type, p_data, p_min, p_max, format, power, ImGuiSliderFlags.None, &grab_bb);
    if (value_changed)
        MarkItemEdited(id);
...
bool SliderScalarN(string label, ImGuiDataType data_type, void* v, int components, const void* p_min, const void* p_max, string format = NULL, float power = 1.0f)
{
    ...
    for (int i = 0; i < components; i++)
    {
        ...
        if (i > 0)
            SameLine(0, g.Style.ItemInnerSpacing.x);
        value_changed |= SliderScalar("", data_type, v, p_min, p_max, format, power);
        PopID();
        PopItemWidth();
...
bool SliderFloat2(string label, ImVec2* v, float v_min, float v_max, string format = "%.3f", float power = 1.0f)
{
...
bool VSliderScalar(string label, const ImVec2/*&*/ size, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, string format = NULL, float power = 1.0f)
{
    ...
    // Slider behavior
    ImRect grab_bb;
    const bool value_changed = SliderBehavior(frame_bb, id, data_type, p_data, p_min, p_max, format, power, ImGuiSliderFlags.Vertical, &grab_bb);
    if (value_changed)
        MarkItemEdited(id);
...
int ImParseFormatPrecision(string fmt, int default_precision)
{
    ...
    if (fmt_start < fmt.length && fmt[fmt_start] == '.')
    {
        fmt_start += ImAtoi!int(fmt[fmt_start..$], &precision).length;
...
bool TempInputText(const ImRect/*&*/ bb, ImGuiID id, string label, char[] buf, ImGuiInputTextFlags flags)
{
    ...
    bool value_changed = InputTextEx(label, NULL, buf, bb.GetSize(), flags);
    if (init)
    {
...
bool InputScalar(string label, ImGuiDataType data_type, void* p_data, const void* p_step = NULL, const void* p_step_fast = NULL, string format = NULL, ImGuiInputTextFlags flags = ImGuiInputTextFlags.None)
{
    ...
    if (p_step != NULL)
    {
        ...
        if (flags & ImGuiInputTextFlags.ReadOnly)
            button_flags |= ImGuiButtonFlags.Disabled;
...
bool InputFloat(string label, float* v, float step, float step_fast, int decimal_precision, ImGuiInputTextFlags flags)
{
    ...
    return InputFloat(label, v, step, step_fast, cast(string)format[0..length], flags);
}

bool InputFloat2(string label, float[/*2*/] v, int decimal_precision, ImGuiInputTextFlags flags)
{
    char[16] format = "%f";
    size_t length = 2;
    if (decimal_precision >= 0)
...
ImWchar STB_TEXTEDIT_GETCHAR(const STB_TEXTEDIT_STRING* obj, int idx)                      { return obj.TextW[idx]; }
float   STB_TEXTEDIT_GETWIDTH(STB_TEXTEDIT_STRING* obj, int line_start_idx, int char_idx)  { ImWchar c = obj.TextW[line_start_idx + char_idx]; if (c == '\n') return STB_TEXTEDIT_GETWIDTH_NEWLINE; ImGuiContext* g = GImGui; return g.Font.GetCharAdvance(c) * (g.FontSize / g.Font.FontSize); }
...
{
    const (ImWchar)* text = obj.TextW.asArray().ptr;
...
{
    const bool is_resizable = (obj.UserFlags & ImGuiInputTextFlags.CallbackResize) != 0;
...
void stb_textedit_replace(STB_TEXTEDIT_STRING* str, STB_TexteditState* state, const STB_TEXTEDIT_CHARTYPE* text, int text_len)
{
...
void InsertChars(int pos, string new_text)
{
    ...
    memcpy(Buf + pos, new_text.ptr, cast(size_t)new_text_len * sizeof!(char));
...
bool InputTextFilterCharacter(uint* p_char, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data)
{
    uint c = *p_char;
    ...
    if (flags & (ImGuiInputTextFlags.CharsDecimal | ImGuiInputTextFlags.CharsHexadecimal | ImGuiInputTextFlags.CharsUppercase | ImGuiInputTextFlags.CharsNoBlank | ImGuiInputTextFlags.CharsScientific))
    {
        // Allow 0-9 . - + * /
...
bool InputTextEx(string label, string hint, char[] buf, const ImVec2/*&*/ size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback = NULL, void* callback_user_data = NULL)
{
    ...
    if (is_multiline)
    {
        ...
        bool child_visible = BeginChildEx(label, id, frame_bb.GetSize(), true, ImGuiWindowFlags.NoMove | ImGuiWindowFlags.AlwaysUseWindowPadding);
        PopStyleVar(3);
        PopStyleColor();
        if (!child_visible)
        {
    ...
    const bool user_nav_input_start = (g.ActiveId != id) && ((g.NavInputId == id) || (g.NavActivateId == id && g.NavInputSource == ImGuiInputSource.NavKeyboard));
    ...
    if (init_state && g.ActiveId != id)
    {
        ...
        if (flags & ImGuiInputTextFlags.AlwaysInsertMode)
    ...
    if (g.ActiveId != id && init_make_active)
    {
        ...
        if (is_multiline)
            g.ActiveIdUsingKeyInputMask |= (cast(ImU64)1 << ImGuiKey.PageUp) | (cast(ImU64)1 << ImGuiKey.PageDown); // FIXME-NAV: Page up/down actually not supported yet by widget, but claim them ahead.
    ...
    if (g.ActiveId == id)
    {
        ...
        // When using 'ImGuiInputTextFlags.EnterReturnsTrue' as a special case we reapply the live buffer back to the input buffer before clearing ActiveId, even though strictly speaking it wasn't modified on this frame.
        // If we didn't do that, code like InputInt() with ImGuiInputTextFlags.EnterReturnsTrue would fail.
        // This also allows the user to use InputText() with ImGuiInputTextFlags.EnterReturnsTrue without maintaining any user-side storage (please note that if you use this property along ImGuiInputTextFlags.CallbackResize you can end up with your temporary string object unnecessarily allocating once a frame, either store your string data, either if you don't then don't use ImGuiInputTextFlags.CallbackResize).
        ...
        if (apply_edit_back_to_user_buffer)
        {
            ...
            // User callback
            if ((flags & (ImGuiInputTextFlags.CallbackCompletion | ImGuiInputTextFlags.CallbackHistory | ImGuiInputTextFlags.CallbackAlways)) != 0)
            {
                ...
                if (event_flag)
                {
                    ...
                    if (callback_data.CursorPos != utf8_cursor_pos)            { state.Stb.cursor = ImTextCountCharsFromUtf8(cast(string)callback_data.Buf[0..callback_data.CursorPos]); state.CursorFollow = true; }
                    if (callback_data.SelectionStart != utf8_selection_start)  { state.Stb.select_start = ImTextCountCharsFromUtf8(cast(string)callback_data.Buf[0..callback_data.SelectionStart]); }
                    if (callback_data.SelectionEnd != utf8_selection_end)      { state.Stb.select_end = ImTextCountCharsFromUtf8(cast(string)callback_data.Buf[0..callback_data.SelectionEnd]); }
        ...
        state.UserFlags = ImGuiInputTextFlags.None;
...
bool ColorEdit4(string label, float[/*4*/] col, ImGuiColorEditFlags flags = ImGuiColorEditFlags.None)
{
    ...
    if (flags & ImGuiColorEditFlags.NoInputs)
        flags = (flags & (~ImGuiColorEditFlags._DisplayMask)) | ImGuiColorEditFlags.DisplayRGB | ImGuiColorEditFlags.NoOptions;
    ...
    // Read stored options
    if (!(flags & ImGuiColorEditFlags._DisplayMask))
        flags |= (g.ColorEditOptions & ImGuiColorEditFlags._DisplayMask);
    if (!(flags & ImGuiColorEditFlags._DataTypeMask))
        flags |= (g.ColorEditOptions & ImGuiColorEditFlags._DataTypeMask);
    if (!(flags & ImGuiColorEditFlags._PickerMask))
        flags |= (g.ColorEditOptions & ImGuiColorEditFlags._PickerMask);
    if (!(flags & ImGuiColorEditFlags._InputMask))
        flags |= (g.ColorEditOptions & ImGuiColorEditFlags._InputMask);
    flags |= (g.ColorEditOptions & ~(ImGuiColorEditFlags._DisplayMask | ImGuiColorEditFlags._DataTypeMask | ImGuiColorEditFlags._PickerMask | ImGuiColorEditFlags._InputMask));
    IM_ASSERT(ImIsPowerOfTwo(flags & ImGuiColorEditFlags._DisplayMask)); // Check that only 1 is selected
    IM_ASSERT(ImIsPowerOfTwo(flags & ImGuiColorEditFlags._InputMask));   // Check that only 1 is selected
...
bool ColorPicker4(string label, float[/*4*/] col, ImGuiColorEditFlags flags = ImGuiColorEditFlags.None, const float* ref_col = NULL)
{
    ...
    // Read stored options
    if (!(flags & ImGuiColorEditFlags._PickerMask))
        flags |= ((g.ColorEditOptions & ImGuiColorEditFlags._PickerMask) ? g.ColorEditOptions : ImGuiColorEditFlags._OptionsDefault) & ImGuiColorEditFlags._PickerMask;
    if (!(flags & ImGuiColorEditFlags._InputMask))
        flags |= ((g.ColorEditOptions & ImGuiColorEditFlags._InputMask) ? g.ColorEditOptions : ImGuiColorEditFlags._OptionsDefault) & ImGuiColorEditFlags._InputMask;
    IM_ASSERT(ImIsPowerOfTwo(flags & ImGuiColorEditFlags._PickerMask)); // Check that only 1 is selected
    IM_ASSERT(ImIsPowerOfTwo(flags & ImGuiColorEditFlags._InputMask));  // Check that only 1 is selected
    if (!(flags & ImGuiColorEditFlags.NoOptions))
    ...
    PopItemFlag(); // ImGuiItemFlags.NoNav
    ...
    if ((flags & ImGuiColorEditFlags.NoInputs) == 0)
    {
        ...
        ImGuiColorEditFlags sub_flags_to_forward = ImGuiColorEditFlags._DataTypeMask | ImGuiColorEditFlags._InputMask | ImGuiColorEditFlags.HDR | ImGuiColorEditFlags.NoAlpha | ImGuiColorEditFlags.NoOptions | ImGuiColorEditFlags.NoSmallPreview | ImGuiColorEditFlags.AlphaPreview | ImGuiColorEditFlags.AlphaPreviewHalf;
        ImGuiColorEditFlags sub_flags = (flags & sub_flags_to_forward) | ImGuiColorEditFlags.NoPicker;
        if (flags & ImGuiColorEditFlags.DisplayRGB || (flags & ImGuiColorEditFlags._DisplayMask) == 0)
        ...
        if (flags & ImGuiColorEditFlags.DisplayHSV || (flags & ImGuiColorEditFlags._DisplayMask) == 0)
            value_changed |= ColorEdit4("##hsv", col, sub_flags | ImGuiColorEditFlags.DisplayHSV);
        if (flags & ImGuiColorEditFlags.DisplayHex || (flags & ImGuiColorEditFlags._DisplayMask) == 0)
...
bool ColorButton(string desc_id, const ImVec4/*&*/ col, ImGuiColorEditFlags flags = ImGuiColorEditFlags.None, ImVec2 size = ImVec2(0, 0))
{
    ...
    if ((flags & ImGuiColorEditFlags.AlphaPreviewHalf) && col_rgb.w < 1.0f)
    {
        ...
        RenderColorRectWithAlphaCheckerboard(window.DrawList,ImVec2(bb_inner.Min.x + grid_step, bb_inner.Min.y), bb_inner.Max, GetColorU32(col_rgb), grid_step, ImVec2(-grid_step + off, off), rounding, ImDrawCornerFlags.TopRight| ImDrawCornerFlags.BotRight);
        window.DrawList.AddRectFilled(bb_inner.Min, ImVec2(mid_x, bb_inner.Max.y), GetColorU32(col_rgb_without_alpha), rounding, ImDrawCornerFlags.TopLeft|ImDrawCornerFlags.BotLeft);
    }
    else
    {
        ...
        if (col_source.w < 1.0f)
            ...
            window.DrawList.AddRectFilled(bb_inner.Min, bb_inner.Max, GetColorU32(col_source), rounding, ImDrawCornerFlags.All);
    }
    RenderNavHighlight(bb, id);
    if ((flags & ImGuiColorEditFlags.NoBorder) == 0)
    {
    ...
    if (!(flags & ImGuiColorEditFlags.NoTooltip) && hovered)
        ColorTooltip(desc_id, &col.x, flags & (ImGuiColorEditFlags._InputMask | ImGuiColorEditFlags.NoAlpha | ImGuiColorEditFlags.AlphaPreview | ImGuiColorEditFlags.AlphaPreviewHalf));
...
{
    ImGuiContext* g = GImGui;
    if ((flags & ImGuiColorEditFlags._DisplayMask) == 0)
        flags |= ImGuiColorEditFlags._OptionsDefault & ImGuiColorEditFlags._DisplayMask;
    if ((flags & ImGuiColorEditFlags._DataTypeMask) == 0)
        flags |= ImGuiColorEditFlags._OptionsDefault & ImGuiColorEditFlags._DataTypeMask;
    if ((flags & ImGuiColorEditFlags._PickerMask) == 0)
        flags |= ImGuiColorEditFlags._OptionsDefault & ImGuiColorEditFlags._PickerMask;
    if ((flags & ImGuiColorEditFlags._InputMask) == 0)
        flags |= ImGuiColorEditFlags._OptionsDefault & ImGuiColorEditFlags._InputMask;
    IM_ASSERT(ImIsPowerOfTwo(cast(int)(flags & ImGuiColorEditFlags._DisplayMask)));    // Check only 1 option is selected
    IM_ASSERT(ImIsPowerOfTwo(cast(int)(flags & ImGuiColorEditFlags._DataTypeMask)));   // Check only 1 option is selected
    IM_ASSERT(ImIsPowerOfTwo(cast(int)(flags & ImGuiColorEditFlags._PickerMask)));     // Check only 1 option is selected
    IM_ASSERT(ImIsPowerOfTwo(cast(int)(flags & ImGuiColorEditFlags._InputMask)));      // Check only 1 option is selected
...
void ColorTooltip(string text, const float* col, ImGuiColorEditFlags flags)
{
    ...
    ColorButton("##preview", cf, (flags & (ImGuiColorEditFlags._InputMask | ImGuiColorEditFlags.NoAlpha | ImGuiColorEditFlags.AlphaPreview | ImGuiColorEditFlags.AlphaPreviewHalf)) | ImGuiColorEditFlags.NoTooltip, sz);
    SameLine();
    if ((flags & ImGuiColorEditFlags.InputRGB) || !(flags & ImGuiColorEditFlags._InputMask))
    {
        if (flags & ImGuiColorEditFlags.NoAlpha)
...
{
    bool allow_opt_inputs = !(flags & ImGuiColorEditFlags._DisplayMask);
    bool allow_opt_datatype = !(flags & ImGuiColorEditFlags._DataTypeMask);
    ...
    if (allow_opt_inputs)
    {
        if (RadioButton("RGB", (opts & ImGuiColorEditFlags.DisplayRGB) != 0)) opts = (opts & ~ImGuiColorEditFlags._DisplayMask) | ImGuiColorEditFlags.DisplayRGB;
        if (RadioButton("HSV", (opts & ImGuiColorEditFlags.DisplayHSV) != 0)) opts = (opts & ~ImGuiColorEditFlags._DisplayMask) | ImGuiColorEditFlags.DisplayHSV;
        if (RadioButton("Hex", (opts & ImGuiColorEditFlags.DisplayHex) != 0)) opts = (opts & ~ImGuiColorEditFlags._DisplayMask) | ImGuiColorEditFlags.DisplayHex;
    }
    if (allow_opt_datatype)
    {
        if (allow_opt_inputs) Separator();
        if (RadioButton("0..255",     (opts & ImGuiColorEditFlags.Uint8) != 0)) opts = (opts & ~ImGuiColorEditFlags._DataTypeMask) | ImGuiColorEditFlags.Uint8;
        if (RadioButton("0.00..1.00", (opts & ImGuiColorEditFlags.Float) != 0)) opts = (opts & ~ImGuiColorEditFlags._DataTypeMask) | ImGuiColorEditFlags.Float;
    }
...
{
    bool allow_opt_picker = !(flags & ImGuiColorEditFlags._PickerMask);
    ...
    if (allow_opt_picker)
    {
        ...
        for (int picker_type = 0; picker_type < 2; picker_type++)
        {
            ...
            if (Selectable("##selectable", false, ImGuiSelectableFlags.None, picker_size)) // By default, Selectable() is closing popup
                g.ColorEditOptions = (g.ColorEditOptions & ~ImGuiColorEditFlags._PickerMask) | (picker_flags & ImGuiColorEditFlags._PickerMask);
            SetCursorScreenPos(backup_pos);
            ImVec4 dummy_ref_col;
            memcpy(&dummy_ref_col, ref_col, sizeof!(float) * (picker_flags & ImGuiColorEditFlags.NoAlpha ? 3 : 4));
    ...
    if (allow_opt_alpha_bar)
    {
        ...
        CheckboxFlags("Alpha Bar", cast(uint*)&g.ColorEditOptions, ImGuiColorEditFlags.AlphaBar);
    }
    EndPopup();
}
...
bool TreeNodeBehaviorIsOpen(ImGuiID id, ImGuiTreeNodeFlags flags = ImGuiTreeNodeFlags.None)
{
    ...
    if (g.NextItemData.Flags & ImGuiNextItemDataFlags.HasOpen)
    {
        ...
        {
            // We treat ImGuiCond.Once and ImGuiCond.FirstUseEver the same because tree node state are not saved persistently.
...
bool TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, string label)
{
    ...
    window.DC.LastItemStatusFlags |= ImGuiItemStatusFlags.HasDisplayRect;
    ...
    if (!item_add)
    {
        ...
        IMGUI_TEST_ENGINE_ITEM_INFO(window.DC.LastItemId, label, cast(ImGuiItemStatusFlags)(window.DC.ItemFlags | (is_leaf ? 0 : ImGuiItemStatusFlags.Openable) | (is_open ? ImGuiItemStatusFlags.Opened : 0)));
        return is_open;
    }
    ...
    if (is_mouse_x_over_arrow && (flags & ImGuiTreeNodeFlags.OpenOnArrow))
    ...
    if (!is_leaf)
    {
        ...
        if (toggled)
        {
            ...
            window.DC.LastItemStatusFlags |= ImGuiItemStatusFlags.ToggledOpen;
        }
    }
    if (flags & ImGuiTreeNodeFlags.AllowItemOverlap)
        SetItemAllowOverlap();
    ...
    if (selected != was_selected) //-V547
        window.DC.LastItemStatusFlags |= ImGuiItemStatusFlags.ToggledSelection;
    ...
    if (display_frame)
    {
        ...
        if (g.LogEnabled)
        {
            ...
            string log_prefix = "\n##";
            string log_suffix = "##";
            LogRenderedText(&text_pos, log_prefix);
            RenderTextClipped(text_pos, frame_bb.Max, label_end, &label_size);
            LogRenderedText(&text_pos, log_suffix);
        }
        else
        {
            RenderTextClipped(text_pos, frame_bb.Max, label_end, &label_size);
...
bool CollapsingHeader(string label, bool* p_open, ImGuiTreeNodeFlags flags = ImGuiTreeNodeFlags.None)
{
    ...
    if (p_open != NULL)
    {
        ...
        if (CloseButton(window.GetID(cast(void*)(cast(intptr_t)id + 1)), ImVec2(button_x, button_y)))
            *p_open = false;
...
bool Selectable(string label, bool selected = false, ImGuiSelectableFlags flags = ImGuiSelectableFlags.None, const ImVec2/*&*/ size_arg = ImVec2(0, 0))
{
    ...
    if ((flags & ImGuiSelectableFlags.SpanAllColumns) && window.DC.CurrentColumns) // FIXME-OPT: Avoid if vertically clipped.
        PushColumnsBackground();
    ...
    const float min_x = (flags & ImGuiSelectableFlags.SpanAllColumns) ? window.ContentRegionRect.Min.x : pos.x;
    const float max_x = (flags & ImGuiSelectableFlags.SpanAllColumns) ? window.ContentRegionRect.Max.x : GetContentRegionMaxAbs().x;
    ...
    ImRect bb_enlarged = ImRect(min_x, pos.y, text_max.x, text_max.y);
    ...
    bool item_add;
    if (flags & ImGuiSelectableFlags.Disabled)
    {
        ImGuiItemFlags backup_item_flags = window.DC.ItemFlags;
        window.DC.ItemFlags |= ImGuiItemFlags.Disabled | ImGuiItemFlags.NoNavDefaultFocus;
    ...
    if (!item_add)
    {
        if ((flags & ImGuiSelectableFlags.SpanAllColumns) && window.DC.CurrentColumns)
            PopColumnsBackground();
        return false;
    }
    ...
    if (flags & ImGuiSelectableFlags.Disabled)          { button_flags |= ImGuiButtonFlags.Disabled; }
    ...
    if (hovered || selected)
    {
        ...
        RenderNavHighlight(bb_enlarged, id, ImGuiNavHighlightFlags.TypeThin | ImGuiNavHighlightFlags.NoRounding);
    }

    if ((flags & ImGuiSelectableFlags.SpanAllColumns) && window.DC.CurrentColumns)
        PopColumnsBackground();

    if (flags & ImGuiSelectableFlags.Disabled) PushStyleColor(ImGuiCol.Text, style.Colors[ImGuiCol.TextDisabled]);
    RenderTextClipped(text_min, text_max, label, &label_size, style.SelectableTextAlign, &bb_enlarged);
    if (flags & ImGuiSelectableFlags.Disabled) PopStyleColor();

    // Automatically close popups
    if (pressed && (window.Flags & ImGuiWindowFlags.Popup) && !(flags & ImGuiSelectableFlags.DontClosePopups) && !(window.DC.ItemFlags & ImGuiItemFlags.SelectableDontClosePopup))
        CloseCurrentPopup();

    IMGUI_TEST_ENGINE_ITEM_INFO(id, label, cast(ImGuiItemStatusFlags)window.DC.ItemFlags);
    return pressed;
}
...
this(bool dummy)
{
    _data.Spacing = Width = NextWidth = 0.0f;
...
float DeclColumns(float w0, float w1, float w2) // not using va_arg because they promote float to double
{
    ...
    for (int i = 0; i < IM_ARRAYSIZE(Pos); i++)
        NextWidth += NextWidths[i] + ((i > 0 && NextWidths[i] > 0.0f) ? _data.Spacing : 0.0f);
...
bool BeginMainMenuBar()
{
    ...
    PushStyleVar(ImGuiStyleVar.WindowMinSize, ImVec2(0, 0));
    ImGuiWindowFlags window_flags = ImGuiWindowFlags.NoTitleBar | ImGuiWindowFlags.NoResize | ImGuiWindowFlags.NoMove | ImGuiWindowFlags.NoScrollbar | ImGuiWindowFlags.NoSavedSettings | ImGuiWindowFlags.MenuBar;
...
bool BeginMenu(string label, bool enabled = true)
{
    ...
    ImVec2 label_size = CalcTextSize(label, true);
    bool pressed;
    bool menuset_is_open = !(window.Flags & ImGuiWindowFlags.Popup) && (g.OpenPopupStack.Size > g.BeginPopupStack.Size && g.OpenPopupStack[g.BeginPopupStack.Size].OpenParentId == window.IDStack[window.IDStack.Size - 1]);
    ...
    if (window.DC.LayoutType == ImGuiLayoutType.Horizontal)
    {
        ...
        pressed = Selectable(label, menu_is_open, ImGuiSelectableFlags.NoHoldingActiveID | ImGuiSelectableFlags.SelectOnClick | ImGuiSelectableFlags.DontClosePopups | (!enabled ? ImGuiSelectableFlags.Disabled : ImGuiSelectableFlags.None), ImVec2(w, 0.0f));
        PopStyleVar();
    ...
    else
    {
        ...
        pressed = Selectable(label, menu_is_open, ImGuiSelectableFlags.NoHoldingActiveID | ImGuiSelectableFlags.SelectOnClick | ImGuiSelectableFlags.DontClosePopups | ImGuiSelectableFlags.SpanAvailWidth | (!enabled ? ImGuiSelectableFlags.Disabled : ImGuiSelectableFlags.None), ImVec2(min_w, 0.0f));
        ImU32 text_col = GetColorU32(enabled ? ImGuiCol.Text : ImGuiCol.TextDisabled);
        RenderArrow(window.DrawList, pos + ImVec2(window.DC.MenuColumns.Pos[2] + extra_w + g.FontSize * 0.30f, 0.0f), text_col, ImGuiDir.Right);
    }
    ...
    if (window.DC.LayoutType == ImGuiLayoutType.Vertical) // (window.Flags & (ImGuiWindowFlags.Popup|ImGuiWindowFlags.ChildMenu))
    {
    ...
    if (menu_is_open)
    {
        SetNextWindowPos(popup_pos, ImGuiCond.Always);
        menu_is_open = BeginPopupEx(id, flags); // menu_is_open can be 'false' when the popup is completely clipped (e.g. zero size display)
    }
    else
    {
...
bool MenuItem(string label, string shortcut = NULL, bool selected = false, bool enabled = true)
{
    ...
    // We've been using the equivalent of ImGuiSelectableFlags.SetNavIdOnHover on all Selectable() since early Nav system days (commit 43ee5d73),
    // but I am unsure whether this should be kept at all. For now moved it to be an opt-in feature used by menus only.
    ImGuiSelectableFlags flags = ImGuiSelectableFlags.SelectOnRelease | ImGuiSelectableFlags.SetNavIdOnHover | (enabled ? ImGuiSelectableFlags.None : ImGuiSelectableFlags.Disabled);
    bool pressed;
    if (window.DC.LayoutType == ImGuiLayoutType.Horizontal)
    {
    ...
    else
    {
        ...
        float extra_w = ImMax(0.0f, GetContentRegionAvail().x - min_w);
        pressed = Selectable(label, false, flags | ImGuiSelectableFlags.SpanAvailWidth, ImVec2(min_w, 0.0f));
        if (shortcut_w > 0.0f)
        {
...
int TabItemComparerByVisibleOffset(const ImGuiTabItem* a, const ImGuiTabItem* b)
{
    return cast(int)(a.Offset - b.Offset);
}

ImGuiTabBar* GetTabBarFromTabBarRef(const ImGuiPtrOrIndex/*&*/ _ref)
{
    ImGuiContext* g = GImGui;
...
bool    BeginTabBarEx(ImGuiTabBar* tab_bar, const ImRect/*&*/ tab_bar_bb, ImGuiTabBarFlags flags)
{
    ...
    if ((flags & ImGuiTabBarFlags.Reorderable) && !(tab_bar.Flags & ImGuiTabBarFlags.Reorderable) && tab_bar.Tabs.Size > 1 && tab_bar.PrevFrameVisible != -1)
        ImQsort(tab_bar.Tabs.asArray(), &TabItemComparerByVisibleOffset);
...
void TabBarLayout(ImGuiTabBar* tab_bar)
{
    ...
    if (tab_bar.ReorderRequestTabId != 0)
    {
        ...
        {
            //IM_ASSERT(tab_bar.Flags & ImGuiTabBarFlags.Reorderable); // <- this may happen when using debug tools
    ...
    if (width_excess > 0.0f && (tab_bar.Flags & ImGuiTabBarFlags.FittingPolicyResizeDown))
    {
    ...
    const bool scrolling_buttons = (tab_bar.OffsetMax > tab_bar.BarRect.GetWidth() && tab_bar.Tabs.Size > 1) && !(tab_bar.Flags & ImGuiTabBarFlags.NoTabListScrollingButtons) && (tab_bar.Flags & ImGuiTabBarFlags.FittingPolicyScroll);
...
{
    if ((tab_bar.VisibleTabId == tab.ID) && !(tab.Flags & ImGuiTabItemFlags.UnsavedDocument))
    {
    ...
    else if ((tab_bar.VisibleTabId != tab.ID) && (tab.Flags & ImGuiTabItemFlags.UnsavedDocument))
    {
...
void TabBarScrollToTab(ImGuiTabBar* tab_bar, ImGuiTabItem* tab)
{
    ImGuiContext* g = GImGui;
...
void TabBarQueueChangeTabOrder(ImGuiTabBar* tab_bar, const ImGuiTabItem* tab, int dir)
{
    ...
    tab_bar.ReorderRequestDir = cast(ImS8)dir;
}
...
ImGuiTabItem* TabBarTabListPopupButton(ImGuiTabBar* tab_bar)
{
    ...
    PushStyleColor(ImGuiCol.Text, arrow_col);
    PushStyleColor(ImGuiCol.Button, ImVec4(0, 0, 0, 0));
    bool open = BeginCombo("##v", NULL, ImGuiComboFlags.NoPreview);
    PopStyleColor(2);
...
bool    TabItemEx(ImGuiTabBar* tab_bar, string label, bool* p_open, ImGuiTabItemFlags flags)
{
    ...
    tab_bar.LastTabItemIdx = cast(short)tab_bar.Tabs.index_from_ptr(tab);
    ...
    if (!tab_appearing && !(tab_bar.Flags & ImGuiTabBarFlags.Reorderable))
    {
    ...
    ImGuiButtonFlags button_flags = (ImGuiButtonFlags.PressedOnClick | ImGuiButtonFlags.AllowItemOverlap);
    if (g.DragDropActive)
        button_flags |= ImGuiButtonFlags.PressedOnDragDropHold;
    bool hovered, held;
    ...
    if (held && !tab_appearing && IsMouseDragging(ImGuiMouseButton.Left))
    {
        if (!g.DragDropActive && (tab_bar.Flags & ImGuiTabBarFlags.Reorderable))
        {
            ...
            {
                if (tab_bar.Flags & ImGuiTabBarFlags.Reorderable)
...
static if (false) {
    ...
    const ImGuiID close_button_id = p_open ? window.GetID(cast(void*)(cast(intptr_t)id + 1)) : 0;
...
void TabItemBackground(ImDrawList* draw_list, const ImRect/*&*/ bb, ImGuiTabItemFlags flags, ImU32 col)
{
    ...
    if (g.Style.TabBorderSize > 0.0f)
    {
        ...
        draw_list.PathStroke(GetColorU32(ImGuiCol.Border), false, g.Style.TabBorderSize);
    }
}

// Render text label (with custom clipping) + Unsaved Document marker + Close Button logic
// We tend to lock style.FramePadding for a given tab-bar, hence the 'frame_padding' parameter.
bool TabItemLabelAndCloseButton(ImDrawList* draw_list, const ImRect/*&*/ bb, ImGuiTabItemFlags flags, ImVec2 frame_padding, string label, ImGuiID tab_id, ImGuiID close_button_id, bool is_contents_visible)
{
    ImGuiContext* g = GImGui;
    ImVec2 label_size = CalcTextSize(label, true);
    if (bb.GetWidth() <= 1.0f)
...
*/
    ...
    string TAB_UNSAVED_MARKER = "*";
    ImRect text_pixel_clip_bb = ImRect(bb.Min.x + frame_padding.x, bb.Min.y + frame_padding.y, bb.Max.x - frame_padding.x, bb.Max.y);
    if (flags & ImGuiTabItemFlags.UnsavedDocument)
    {
        text_pixel_clip_bb.Max.x -= CalcTextSize(TAB_UNSAVED_MARKER, false).x;
        ImVec2 unsaved_marker_pos = ImVec2(ImMin(bb.Min.x + frame_padding.x + label_size.x + 2, text_pixel_clip_bb.Max.x), bb.Min.y + frame_padding.y + IM_FLOOR(-g.FontSize * 0.25f));
        RenderTextClippedEx(draw_list, unsaved_marker_pos, bb.Max - frame_padding, TAB_UNSAVED_MARKER, NULL);
    }
    ImRect text_ellipsis_clip_bb = text_pixel_clip_bb;

    // Close Button
    // We are relying on a subtle and confusing distinction between 'hovered' and 'g.HoveredId' which happens because we are using ImGuiButtonFlags.AllowOverlapMode + SetItemAllowOverlap()
...
__gshared const float COLUMNS_HIT_RECT_HALF_WIDTH = 4.0f;
...
float GetDraggedColumnOffset(ImGuiColumns* columns, int column_index)
{
    ...
    if (columns.Flags & ImGuiColumnsFlags.NoPreserveWidths)
...
float GetColumnWidthEx(ImGuiColumns* columns, int column_index, bool before_resize = false)
{
    if (column_index < 0)
        column_index = columns.Current;
...
void SetColumnOffset(int column_index, float offset)
{
    ...
    const bool preserve_width = !(columns.Flags & ImGuiColumnsFlags.NoPreserveWidths) && (column_index < columns.Count-1);
    const float width = preserve_width ? GetColumnWidthEx(columns, column_index, columns.IsBeingResized) : 0.0f;

    if (!(columns.Flags & ImGuiColumnsFlags.NoForceWithinWindow))
...
ImGuiColumns* FindOrCreateColumns(ImGuiWindow* window, ImGuiID id)
{
    ...
    window.ColumnsStorage.push_back(ImGuiColumns(false));
...
void BeginColumns(string str_id, int columns_count, ImGuiColumnsFlags flags = ImGuiColumnsFlags.None)
{
    ...
    if (columns.Columns.Size == 0)
    {
        ...
        {
            ImGuiColumnData column = ImGuiColumnData(false);
...
void EndColumns()
{
    ...
    if (!(flags & ImGuiColumnsFlags.GrowParentContentsSize))
        window.DC.CursorMaxPos.x = columns.HostCursorMaxPosX;  // Restore cursor max pos, as columns don't grow parent

    // Draw columns borders and handle resize
    // The IsBeingResized flag ensure we preserve pre-resize columns width so back-and-forth are not lossy
    bool is_being_resized = false;
    if (!(flags & ImGuiColumnsFlags.NoBorder) && !window.SkipItems)
    {
        ...
        for (int n = 1; n < columns.Count; n++)
        {
            ...
            if (!(flags & ImGuiColumnsFlags.NoResize))
            {
                ButtonBehavior(column_hit_rect, column_id, &hovered, &held);
                if (hovered || held)
                    g.MouseCursor = ImGuiMouseCursor.ResizeEW;
                if (held && !(column.Flags & ImGuiColumnsFlags.NoResize))
                    dragging_column = n;
            }
...
void Columns(int columns_count = 1, string id = null, bool border = true)
{
    ...
    ImGuiColumnsFlags flags = (border ? ImGuiColumnsFlags.None : ImGuiColumnsFlags.NoBorder);
    //flags |= ImGuiColumnsFlags.NoPreserveWidths; // NB: Legacy behavior
    ImGuiColumns* columns = window.DC.CurrentColumns;
