...
version (IMGUI_ENABLE_TEST_ENGINE) {
    ImGuiContext* g = GImGui;
    IMGUI_TEST_ENGINE_ID_INFO2(id, ImGuiDataType_String, str);
}
    return id;
}
...
static if (false) {
    ...
    if (nav_gamepad_active && g.NavInputSource != ImGuiInputSource.NavGamepad)
    {
    ...
    {
        pragma(inline, true) void NAV_MAP_KEY(ImGuiKey _KEY, ImGuiNavInput _NAV_INPUT, ImGuiContext* g)  { if (IsKeyDown(io.KeyMap[_KEY])) { io.NavInputs[_NAV_INPUT] = 1.0f; g.NavInputSource = ImGuiInputSource.NavKeyboard; } }
    ...
    if (g.NavMoveRequest && g.NavInputSource == ImGuiInputSource.NavGamepad && g.NavLayer == ImGuiNavLayer.Main)
    {
        ImGuiWindow* window = g.NavWindow;
...
void ShowMetricsWindow(bool* p_open = NULL)
{
    ...
    struct Funcs
    {
        ...
        static void NodeTabBar(ImGuiTabBar* tab_bar)
        {
            ...
            ImFormatString(buf, "Tab Bar 0x%08X (%d tabs)%s", tab_bar.ID, tab_bar.Tabs.Size, is_active ? "" : " *Inactive*");
            //IM_UNUSED(p);
            if (!is_active) { PushStyleColor(ImGuiCol.Text, GetStyleColorVec4(ImGuiCol.TextDisabled)); }
            ...
            if (open)
            {
                for (int tab_n = 0; tab_n < tab_bar.Tabs.Size; tab_n++)
                {
                    ...
                    Text("%02d%c Tab 0x%08X '%s' Offset: %.1f, Width: %.1f/%.1f", tab_n, (tab.ID == tab_bar.SelectedTabId) ? '*' : ' ', tab.ID, (tab.NameOffset != -1) ? tab_bar.GetTabName(tab) : "", tab.Offset, tab.Width, tab.ContentWidth);
                    PopID();
                }
                TreePop();
            }
...
version (IMGUI_HAS_DOCK) {
    ...
    {
        string[5] input_source_names = [ "None", "Mouse", "Nav", "NavKeyboard", "NavGamepad" ]; IM_ASSERT(IM_ARRAYSIZE(input_source_names) == ImGuiInputSource.COUNT);
