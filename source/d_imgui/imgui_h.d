...
enum IMGUI_VERSION       = "1.89.7";
enum IMGUI_VERSION_NUM   = 18970;
...
namespace ImGui
{
    ...
    void          PushTabStop(bool tab_stop);                                     // == tab stop enable. Allow focusing using TAB/Shift-TAB, enabled by default but you can disable it for certain widgets
    void          PopTabStop();
    ...
    void          SeparatorText(string label);                               // currently: formatted text with an horizontal line
    ...
    void          SetTooltip(string fmt, ...) IM_FMTARGS(1);                     // set a text-only tooltip. Often used after a ImGui::IsItemHovered() check. Override any previous call to SetTooltip().
    ...
    void          SetItemTooltip(string fmt, ...) IM_FMTARGS(1);                 // set a text-only tooltip if preceeding item was hovered. override any previous call to SetTooltip().
    void          SetItemTooltipV(string fmt, va_list args) IM_FMTLIST(1);
    ...
    bool          SetDragDropPayload(string type, const void* data, size_t sz, ImGuiCond cond = 0);  // type is a user defined string of maximum 32 characters. Strings starting with '_' are reserved for dear imgui internal types. Data is copied and held by imgui. Return true when payload has been accepted.
...
enum ImGuiHoveredFlags : int
{
    ...
    ForTooltip                    = 1 << 11,  // Shortcut for standard flags when using IsItemHovered() + SetTooltip() sequence.

    // (Advanced) Mouse Hovering delays.
    // - generally you can use ImGuiHoveredFlags_ForTooltip to use application-standardized flags.
    // - use those if you need specific overrides.
    Stationary                    = 1 << 12,  // Require mouse to be stationary for style.HoverStationaryDelay (~0.15 sec) _at least one time_. After this, can move on same item/window. Using the stationary test tends to reduces the need for a long delay.
    DelayNone                     = 1 << 13,  // IsItemHovered() only: Return true immediately (default). As this is the default you generally ignore this.
    DelayShort                    = 1 << 14,  // IsItemHovered() only: Return true after style.HoverDelayShort elapsed (~0.15 sec) (shared between items) + requires mouse to be stationary for style.HoverStationaryDelay (once per item).
    DelayNormal                   = 1 << 15,  // IsItemHovered() only: Return true after style.HoverDelayNormal elapsed (~0.40 sec) (shared between items) + requires mouse to be stationary for style.HoverStationaryDelay (once per item).
    NoSharedDelay                 = 1 << 16,  // IsItemHovered() only: Disable shared delay system where moving from one item to the next keeps the previous timer for a short time (standard for tooltips with long delays)
...
//    KeysData_OFFSET        = ImGuiKey.NamedKey_BEGIN,  // Accesses to io.KeysData[] must use (key - ImGuiKey_KeysData_OFFSET) index.
//} else {
...
enum ImGuiButtonFlags : int
{
    ...
    AllowOverlap       = 1 << 12,  // require previous frame HoveredId to either match id or be null before being usable, use along with SetItemAllowOverlap()
...
struct ImVector(T)
{
    ...
    pragma(inline, true) void         reserve_discard(int new_capacity)   { if (new_capacity <= Capacity) return; if (Data) IM_FREE(Data); Data = IM_ALLOC!T(cast(size_t)new_capacity).ptr; Capacity = new_capacity; }
...
static if (!IMGUI_DISABLE_OBSOLETE_FUNCTIONS) {
    ...
    // Input Functions
    void  AddKeyEvent(ImGuiKey key, bool down) { (cast(ImGuiIO_Wrapper*)&this).AddKeyEvent(key, down); }                   // Queue a new key down/up event. Key should be "translated" (as in, generally ImGuiKey_A matches the key end-user would use to emit an 'A' character)
    void  AddKeyAnalogEvent(ImGuiKey key, bool down, float v) { (cast(ImGuiIO_Wrapper*)&this).AddKeyAnalogEvent(key, down, v); }    // Queue a new key down/up event for analog values (e.g. ImGuiKey_Gamepad_ values). Dead-zones should be handled by the backend.
...
struct ImGuiListClipper
{
    ...
    void  IncludeRangeByIndices(int item_begin, int item_end) { (cast(ImGuiListClipper_Wrapper*)&this).IncludeRangeByIndices(item_begin, item_end); } // item_end is exclusive e.g. use (42, 42+1) to make item 42 never clipped.

static if (!IMGUI_DISABLE_OBSOLETE_FUNCTIONS) {
    pragma(inline, true) void ForceDisplayRangeByIndices(int item_begin, int item_end) { IncludeRangeByIndices(item_begin, item_end); } // [renamed in 1.89.6]
...
IM_MSVC_RUNTIME_CHECKS_OFF
static pragma(inline, true) ImVec2  operator*(const ImVec2/*&*/ lhs, const float rhs)     { return ImVec2(lhs.x * rhs, lhs.y * rhs); }
static pragma(inline, true) ImVec2  operator/(const ImVec2/*&*/ lhs, const float rhs)     { return ImVec2(lhs.x / rhs, lhs.y / rhs); }
static pragma(inline, true) ImVec2  operator+(const ImVec2/*&*/ lhs, const ImVec2/*&*/ rhs)   { return ImVec2(lhs.x + rhs.x, lhs.y + rhs.y); }
static pragma(inline, true) ImVec2  operator-(const ImVec2/*&*/ lhs, const ImVec2/*&*/ rhs)   { return ImVec2(lhs.x - rhs.x, lhs.y - rhs.y); }
static pragma(inline, true) ImVec2  operator*(const ImVec2/*&*/ lhs, const ImVec2/*&*/ rhs)   { return ImVec2(lhs.x * rhs.x, lhs.y * rhs.y); }
static pragma(inline, true) ImVec2  operator/(const ImVec2/*&*/ lhs, const ImVec2/*&*/ rhs)   { return ImVec2(lhs.x / rhs.x, lhs.y / rhs.y); }
static pragma(inline, true) ImVec2  operator-(const ImVec2/*&*/ lhs)                      { return ImVec2(-lhs.x, -lhs.y); }
static pragma(inline, true) ImVec2& operator*=(ImVec2& lhs, const float rhs)          { lhs.x *= rhs; lhs.y *= rhs; return lhs; }
static pragma(inline, true) ImVec2& operator/=(ImVec2& lhs, const float rhs)          { lhs.x /= rhs; lhs.y /= rhs; return lhs; }
static pragma(inline, true) ImVec2& operator+=(ImVec2& lhs, const ImVec2/*&*/ rhs)        { lhs.x += rhs.x; lhs.y += rhs.y; return lhs; }
static pragma(inline, true) ImVec2& operator-=(ImVec2& lhs, const ImVec2/*&*/ rhs)        { lhs.x -= rhs.x; lhs.y -= rhs.y; return lhs; }
static pragma(inline, true) ImVec2& operator*=(ImVec2& lhs, const ImVec2/*&*/ rhs)        { lhs.x *= rhs.x; lhs.y *= rhs.y; return lhs; }
static pragma(inline, true) ImVec2& operator/=(ImVec2& lhs, const ImVec2/*&*/ rhs)        { lhs.x /= rhs.x; lhs.y /= rhs.y; return lhs; }
static pragma(inline, true) ImVec4  operator+(const ImVec4/*&*/ lhs, const ImVec4/*&*/ rhs)   { return ImVec4(lhs.x + rhs.x, lhs.y + rhs.y, lhs.z + rhs.z, lhs.w + rhs.w); }
static pragma(inline, true) ImVec4  operator-(const ImVec4/*&*/ lhs, const ImVec4/*&*/ rhs)   { return ImVec4(lhs.x - rhs.x, lhs.y - rhs.y, lhs.z - rhs.z, lhs.w - rhs.w); }
static pragma(inline, true) ImVec4  operator*(const ImVec4/*&*/ lhs, const ImVec4/*&*/ rhs)   { return ImVec4(lhs.x * rhs.x, lhs.y * rhs.y, lhs.z * rhs.z, lhs.w * rhs.w); }
IM_MSVC_RUNTIME_CHECKS_RESTORE
}
...
version (D_IMGUI_USER_DEFINED_COLOR_SHIFT) {} else {
...
struct ImDrawList
{
    ...
    void  AddConvexPolyFilled(const ImVec2* points, int num_points, ImU32 col) { (cast(ImDrawList_Wrapper*)&this).AddConvexPolyFilled(points, num_points, col); }
    void  AddBezierCubic(const ImVec2/*&*/ p1, const ImVec2/*&*/ p2, const ImVec2/*&*/ p3, const ImVec2/*&*/ p4, ImU32 col, float thickness, int num_segments = 0) { (cast(ImDrawList_Wrapper*)&this).AddBezierCubic(p1, p2, p3, p4, col, thickness, num_segments); } // Cubic Bezier (4 control points)
    ...
    pragma(inline, true)    void  PathFillConvex(ImU32 col)                                   { AddConvexPolyFilled(_Path.Data, _Path.Size, col); _Path.Size = 0; }
...
struct ImFont
{
    ...
    void              RenderChar(ImDrawList* draw_list, float size, const ImVec2/*&*/ pos, ImU32 col, ImWchar c) const { (cast(const ImFont_Wrapper*)&this).RenderChar(draw_list, size, pos, col, c); }
    void              RenderText(ImDrawList* draw_list, float size, const ImVec2/*&*/ pos, ImU32 col, const ImVec4/*&*/ clip_rect, string text, float wrap_width = 0.0f, bool cpu_fine_clip = false) const { (cast(const ImFont_Wrapper*)&this).RenderText(draw_list, size, pos, col, clip_rect, text, wrap_width, cpu_fine_clip); }
...
struct ImGuiPlatformImeData
{
    ...
    float   InputLineHeight = 0;    // Line height
...
namespace ImGui
{
    ...
    void      SetItemAllowOverlap();                                              // Use SetNextItemAllowOverlap() before item.
