...
private void ShowDemoWindowWidgets()
{
    ...
    if (ImGui.TreeNode("Basic"))
    {
        ...
        if (ImGui.IsItemHovered())
        {
            ...
            ImGui.PlotLines("Curve", arr.ptr, IM_ARRAYSIZE(arr));
            ImGui.EndTooltip();
        }
...

        {
            ...
            __gshared float f1_2=1.00f, f2=0.0067f;
            ImGui.DragFloat("drag float", &f1_2, 0.005f);
            ImGui.DragFloat("drag small float", &f2, 0.0001f, 0.0f, 0.0f, "%.06f ns");
        }

        {
            __gshared int i1_3=0;
            ImGui.SliderInt("slider int", &i1_3, -1, 3);
            ImGui.SameLine(); HelpMarker("CTRL+click to input value.");

            __gshared float f1_3=0.123f, f2_3=0.0f;
            ImGui.SliderFloat("slider float", &f1_3, 0.0f, 1.0f, "ratio = %.3f");
            ImGui.SliderFloat("slider float (curve)", &f2_3, -10.0f, 10.0f, "%.4f", 2.0f);
            ...
            string[Element.COUNT] element_names = [ "Fire", "Earth", "Air", "Water" ];
            __gshared int current_element = Element.Fire;
            string current_element_name = (current_element >= 0 && current_element < Element.COUNT) ? element_names[current_element] : "Unknown";
            ImGui.SliderInt("slider enum", &current_element, 0, Element.COUNT - 1, current_element_name);
        ...
        {
            // List box
            string[9] listbox_items = [ "Apple", "Banana", "Cherry", "Kiwi", "Mango", "Orange", "Pineapple", "Strawberry", "Watermelon" ];
            __gshared int listbox_item_current = 1;
    ...
    if (ImGui.TreeNode("Trees"))
    {
        ...
        if (ImGui.TreeNode("Advanced, with Selectable nodes"))
        {
            ...
            __gshared int selection_mask = (1 << 2); // Dumb representation of what may be user-side selection state. You may carry selection state inside or outside your objects in whatever format you see fit.
    ...
    if (ImGui.TreeNode("Combo"))
    {
        ...
        __gshared string item_current_1 = items[0];            // Here our selection is a single pointer stored outside the object.
        if (ImGui.BeginCombo("combo 1", item_current_1, flags)) // The second parameter is the label previewed before opening the combo.
        {
        ...
        struct FuncHolder { static bool ItemGetter(void* data, int idx, string* out_str) nothrow @nogc { *out_str = (cast(string*)data)[idx]; return true; } }
        __gshared int item_current_4 = 0;
        ImGui.Combo("combo 4 (function)", &item_current_4, &FuncHolder.ItemGetter, cast(void*)items.ptr, IM_ARRAYSIZE(items));
    ...
    if (ImGui.TreeNode("Selectables"))
    {
        ...
        if (ImGui.TreeNode("Selection State: Multiple Selection"))
        {
            ...
            __gshared bool[5] selection_2 = [ false, false, false, false, false ];
            for (int n = 0; n < 5; n++)
            {
                char[32] buf;
                snprintf(buf, "Object %d", n);
                if (ImGui.Selectable(ImCstring(buf), selection_2[n]))
                {
                    if (!ImGui.GetIO().KeyCtrl)    // Clear selection when CTRL is not held
                        memset(selection_2.ptr, 0, sizeof(selection_2));
                    selection_2[n] ^= 1;
                }
            }
            ImGui.TreePop();
        ...
        if (ImGui.TreeNode("Rendering more text into the same line"))
        {
            ...
            __gshared bool[3] selected_3 = [ false, false, false ];
            ImGui.Selectable("main.c",    &selected_3[0]); ImGui.SameLine(300); ImGui.Text(" 2,345 bytes");
            ImGui.Selectable("Hello.cpp", &selected_3[1]); ImGui.SameLine(300); ImGui.Text("12,345 bytes");
            ImGui.Selectable("Hello.h",   &selected_3[2]); ImGui.SameLine(300); ImGui.Text(" 2,345 bytes");
            ImGui.TreePop();
        }
        ...
        {
            __gshared bool[4*4] selected_5 = [ true, false, false, false, false, true, false, false, false, false, true, false, false, false, false, true ];
            for (int i = 0; i < 4*4; i++)
            {
                ImGui.PushID(i);
                if (ImGui.Selectable("Sailor", &selected_5[i], ImGuiSelectableFlags.None, ImVec2(50,50)))
                {
                    // Note: We _unnecessarily_ test for both x/y and i here only to silence some static analyzer. The second part of each test is unnecessary.
                    int x = i % 4;
                    int y = i / 4;
                    if (x > 0)           { selected_5[i - 1] ^= 1; }
                    if (x < 3 && i < 15) { selected_5[i + 1] ^= 1; }
                    if (y > 0 && i > 3)  { selected_5[i - 4] ^= 1; }
                    if (y < 3 && i < 12) { selected_5[i + 4] ^= 1; }
                }
        ...
        if (ImGui.TreeNode("Alignment"))
        {
            ...
            __gshared bool[3*3] selected_6 = [ true, false, true, false, true, false, true, false, true ];
            for (int y = 0; y < 3; y++)
            {
                for (int x = 0; x < 3; x++)
                {
                    ...
                    char[32] name;
                    snprintf(name, "(%.1f,%.1f)", alignment.x, alignment.y);
                    if (x > 0) ImGui.SameLine();
                    ImGui.PushStyleVar(ImGuiStyleVar.SelectableTextAlign, alignment);
                    ImGui.Selectable(ImCstring(name), &selected_6[3*y+x], ImGuiSelectableFlags.None, ImVec2(80,80));
                    ImGui.PopStyleVar();
                }
            }
            ImGui.TreePop();
    ...
    if (ImGui.TreeNode("Text Input"))
    {
        ...
        if (ImGui.TreeNode("Filtered Text Input"))
        {
            ...
            struct TextFilters { static int FilterImGuiLetters(ImGuiInputTextCallbackData* data) nothrow @nogc { if (data.EventChar < 256 && ImIndexOf("imgui", cast(char)data.EventChar) > 0) return 0; return 1; } }
    ...
    if (ImGui.TreeNode("Plots Widgets"))
    {
        ...
        __gshared float[7] arr_2 = [ 0.6f, 0.1f, 1.0f, 0.5f, 0.92f, 0.1f, 0.2f ];
        ImGui.PlotLines("Frame Times", arr_2, IM_ARRAYSIZE(arr_2));
        ...
        // (in this example, we will display an average value)
        {
            ...
            ImGui.PlotLines("Lines", values, values_offset, ImCstring(overlay), -1.0f, 1.0f, ImVec2(0,80));
        }
        ImGui.PlotHistogram("Histogram", arr_2, 0, NULL, 0.0f, 1.0f, ImVec2(0,80));
        ...
        float function(void*, int) nothrow @nogc func = (func_type == 0) ? &Funcs_2.Sin : &Funcs_2.Saw;
        ...
        char[32] buf;
        snprintf(buf, "%d/%d", cast(int)(progress_saturated*1753), 1753);
    ...
    if (ImGui.TreeNode("Color/Picker Widgets"))
    {
        ...
        if (ImGui.BeginPopup("mypicker"))
        {
            ...
            for (int n = 0; n < IM_ARRAYSIZE(saved_palette); n++)
            {
                ...
                if (ImGui.ColorButton("##palette", saved_palette[n], ImGuiColorEditFlags.NoAlpha | ImGuiColorEditFlags.NoPicker | ImGuiColorEditFlags.NoTooltip, ImVec2(20,20)))
        ...
        __gshared ImVec4 color_stored_as_hsv = ImVec4(0.23f, 1.0f, 1.0f, 1.0f);
        ImGui.Spacing();
        ...
        ImGui.ColorEdit4("HSV shown as RGB##1", &color_stored_as_hsv, ImGuiColorEditFlags.DisplayRGB | ImGuiColorEditFlags.InputHSV | ImGuiColorEditFlags.Float);
        ImGui.ColorEdit4("HSV shown as HSV##1", &color_stored_as_hsv, ImGuiColorEditFlags.DisplayHSV | ImGuiColorEditFlags.InputHSV | ImGuiColorEditFlags.Float);
        ImGui.DragFloat4("Raw HSV values", color_stored_as_hsv.array(), 0.01f, 0.0f, 1.0f);
    ...
    if (ImGui.TreeNode("Data Types"))
    {
        ...
        ImGui.DragScalar("drag float",     ImGuiDataType.Float,  &f32_v, 0.005f,  &f32_zero, &f32_one, "%f", 1.0f);
        ImGui.DragScalar("drag float ^2",  ImGuiDataType.Float,  &f32_v, 0.005f,  &f32_zero, &f32_one, "%f", 2.0f); ImGui.SameLine(); HelpMarker("You can use the 'power' parameter to increase tweaking precision on one side of the range.");
        ImGui.DragScalar("drag double",    ImGuiDataType.Double, &f64_v, 0.0005f, &f64_zero, NULL,     "%.10f grams", 1.0f);
        ImGui.DragScalar("drag double ^2", ImGuiDataType.Double, &f64_v, 0.0005f, &f64_zero, &f64_one, "0 < %.10f < 1", 2.0f);
        ...
        ImGui.SliderScalar("slider s64 low",     ImGuiDataType.S64,    &s64_v, &s64_zero, &s64_fifty,"%I64d");
        ImGui.SliderScalar("slider s64 high",    ImGuiDataType.S64,    &s64_v, &s64_hi_a, &s64_hi_b, "%I64d");
        ImGui.SliderScalar("slider s64 full",    ImGuiDataType.S64,    &s64_v, &s64_min,  &s64_max,  "%I64d");
        ImGui.SliderScalar("slider u64 low",     ImGuiDataType.U64,    &u64_v, &u64_zero, &u64_fifty,"%I64u ms");
        ImGui.SliderScalar("slider u64 high",    ImGuiDataType.U64,    &u64_v, &u64_hi_a, &u64_hi_b, "%I64u ms");
        ImGui.SliderScalar("slider u64 full",    ImGuiDataType.U64,    &u64_v, &u64_min,  &u64_max,  "%I64u ms");
        ImGui.SliderScalar("slider float low",   ImGuiDataType.Float,  &f32_v, &f32_zero, &f32_one);
        ImGui.SliderScalar("slider float low^2", ImGuiDataType.Float,  &f32_v, &f32_zero, &f32_one,  "%.10f", 2.0f);
        ImGui.SliderScalar("slider float high",  ImGuiDataType.Float,  &f32_v, &f32_lo_a, &f32_hi_a, "%e");
        ImGui.SliderScalar("slider double low",  ImGuiDataType.Double, &f64_v, &f64_zero, &f64_one,  "%.10f grams", 1.0f);
        ImGui.SliderScalar("slider double low^2",ImGuiDataType.Double, &f64_v, &f64_zero, &f64_one,  "%.10f", 2.0f);
        ImGui.SliderScalar("slider double high", ImGuiDataType.Double, &f64_v, &f64_lo_a, &f64_hi_a, "%e grams", 1.0f);
    ...
    if (ImGui.TreeNode("Vertical Sliders"))
    {
        ...
        __gshared float[7] values_2 = [ 0.0f, 0.60f, 0.35f, 0.9f, 0.70f, 0.20f, 0.0f ];
        ImGui.PushID("set1");
        for (int i = 0; i < 7; i++)
        {
            ...
            ImGui.VSliderFloat("##v", ImVec2(18,160), &values_2[i], 0.0f, 1.0f, "");
            if (ImGui.IsItemActive() || ImGui.IsItemHovered())
                ImGui.SetTooltip("%.3f", values_2[i]);
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            ImGui.VSliderFloat("##v", ImVec2(40,160), &values_2[i], 0.0f, 1.0f, "%.2f\nsec");
            ImGui.PopStyleVar();
            ImGui.PopID();
        }
        ImGui.PopID();
    ...
    if (ImGui.TreeNode("Drag and Drop"))
    {
        if (ImGui.TreeNode("Drag and drop in standard widgets"))
        {
            ...
            ImGui.ColorEdit4("color 2", col2_2);
            ImGui.TreePop();
        }
        ...
        if (ImGui.TreeNode("Drag and drop to copy/swap items"))
        {
            ...
            __gshared string[9] names = [ "Bobby", "Beatrice", "Betty", "Brianna", "Barry", "Bernard", "Bibi", "Blaine", "Bryn" ];
            for (int n = 0; n < IM_ARRAYSIZE(names); n++)
            {
                ...
                if (ImGui.BeginDragDropSource(ImGuiDragDropFlags.None))
                {
                    ...
                    if (mode == Mode.Copy) { ImGui.Text("Copy %s", names[n]); }    // Display preview (could be anything, e.g. when dragging an image we could decide to display the filename and a small preview of the image, etc.)
    ...
    if (ImGui.TreeNode("Querying Status (Active/Focused/Hovered etc.)"))
    {
        ...
        __gshared char[66] dummy_str = "This is a dummy field to be able to tab-out of the widgets above.";
        ImGui.InputText("dummy", dummy_str, ImGuiInputTextFlags.ReadOnly);
...
private void ShowDemoWindowLayout()
{
    ...
    if (ImGui.TreeNode("Child windows"))
    {
        ...
        __gshared int line = 50;
        bool goto_line = ImGui.Button("Goto");
        ImGui.SameLine();
        ImGui.SetNextItemWidth(100);
        goto_line |= ImGui.InputInt("##Line", &line, 0, 0, ImGuiInputTextFlags.EnterReturnsTrue);

        // Child 1: no border, enable horizontal scrollbar
        {
            ImGuiWindowFlags window_flags = ImGuiWindowFlags.HorizontalScrollbar | (disable_mouse_wheel ? ImGuiWindowFlags.NoScrollWithMouse : ImGuiWindowFlags.None);
        ...
        {
            ImGuiWindowFlags window_flags = (disable_mouse_wheel ? ImGuiWindowFlags.NoScrollWithMouse : ImGuiWindowFlags.None) | (disable_menu ? ImGuiWindowFlags.None : ImGuiWindowFlags.MenuBar);
    ...
    if (ImGui.TreeNode("Scrolling"))
    {
        ...
        for (int i = 0; i < 5; i++)
        {
            ...
            ImGuiWindowFlags child_flags = enable_extra_decorations ? ImGuiWindowFlags.MenuBar : ImGuiWindowFlags.None;
            bool window_visible = ImGui.BeginChild(ImGui.GetID(cast(void*)cast(size_t)i), ImVec2(child_w, 200.0f), true, child_flags);
            if (ImGui.BeginMenuBar())
            {
        ...
        for (int i = 0; i < 5; i++)
        {
            ...
            bool window_visible = ImGui.BeginChild(ImGui.GetID(cast(void*)cast(size_t)i), ImVec2(-100, child_height), true, child_flags);
            if (scroll_to_off)
        ...
        ImGui.BeginChild("scrolling", ImVec2(0, ImGui.GetFrameHeightWithSpacing() * 7 + 30), true, ImGuiWindowFlags.HorizontalScrollbar);
    ...
    {
        __gshared ImVec2 size = ImVec2(100, 100), offset = ImVec2(50, 20);
        ImGui.TextWrapped("On a per-widget basis we are occasionally clipping text CPU-side if it won't fit in its frame. Otherwise we are doing coarser clipping + passing a scissor rectangle to the renderer. The system is designed to try minimizing both execution and CPU/GPU rendering cost.");
        ImGui.DragFloat2("size", (cast(float*)&size)[0..2], 0.5f, 1.0f, 200.0f, "%.0f");
        ImGui.TextWrapped("(Click and drag)");
        ImVec2 pos = ImGui.GetCursorScreenPos();
        ImVec4 clip_rect = ImVec4(pos.x, pos.y, pos.x + size.x, pos.y + size.y);
        ImGui.InvisibleButton("##dummy", size);
        if (ImGui.IsItemActive() && ImGui.IsMouseDragging(ImGuiMouseButton.Left)) { offset.x += ImGui.GetIO().MouseDelta.x; offset.y += ImGui.GetIO().MouseDelta.y; }
...
private void ShowDemoWindowPopups()
{
    ...
    if (ImGui.TreeNode("Context menus"))
    {
        ...
        ImGui.OpenPopupOnItemClick("item context menu", ImGuiMouseButton.Right);

        // When used after an item that has an ID (here the Button), we can skip providing an ID to BeginPopupContextItem().
        // BeginPopupContextItem() will use the last item ID as the popup ID.
        // In addition here, we want to include your editable label inside the button label. We use the ### operator to override the ID (read FAQ about ID for details)
        __gshared char[32] name = "Label1";
        char[64] buf; snprintf(buf, "Button: %s###Button", name); // ### operator override ID ignoring the preceding label
...
private void ShowDemoWindowColumns()
{
    ...
    */
    ...
    if (ImGui.TreeNode("Horizontal Scrolling"))
    {
        ...
        ImGui.BeginChild("##ScrollingRegion", ImVec2(0, ImGui.GetFontSize() * 20), false, ImGuiWindowFlags.HorizontalScrollbar);
        ImGui.Columns(10);
...
private void ShowDemoWindowMisc()
{
    ...
    if (ImGui.CollapsingHeader("Inputs, Navigation & Focus"))
    {
        ...
        if (ImGui.TreeNode("Keyboard, Mouse & Navigation State"))
        {
            ...
            ImGui.Text("Mouse dbl-clicked:"); for (int i = 0; i < IM_ARRAYSIZE(io.MouseDown); i++) if (ImGui.IsMouseDoubleClicked(cast(ImGuiMouseButton)i)) { ImGui.SameLine(); ImGui.Text("b%d", i); }
        ...
        if (ImGui.TreeNode("Focus from code"))
        {
            ...
            int has_focus = 0;
            __gshared char[128] buf_2 = "click on a button to set focus";

            if (focus_1) ImGui.SetKeyboardFocusHere();
            ImGui.InputText("1", buf_2);
            if (ImGui.IsItemActive()) has_focus = 1;

            if (focus_2) ImGui.SetKeyboardFocusHere();
            ImGui.InputText("2", buf_2);
        ...
        if (ImGui.TreeNode("Dragging"))
        {
            ...
            for (int button = 0; button < 3; button++)
                ImGui.Text("IsMouseDragging(%d):\n  w/ default threshold: %d,\n  w/ zero threshold: %d\n  w/ large threshold: %d",
                    button, ImGui.IsMouseDragging(cast(ImGuiMouseButton)button), ImGui.IsMouseDragging(cast(ImGuiMouseButton)button, 0.0f), ImGui.IsMouseDragging(cast(ImGuiMouseButton)button, 20.0f));
...
void ShowAboutWindow(bool* p_open = NULL)
{
    ...
    if (show_config_info)
    {
        ...
        ImGui.BeginChildFrame(ImGui.GetID("cfginfos"), ImVec2(0, ImGui.GetTextLineHeightWithSpacing() * 18), ImGuiWindowFlags.NoMove);
...
void ShowStyleEditor(ImGuiStyle* _ref = NULL)
{
    ...
    ImGuiStyle* style = &ImGui.GetStyle();
    __gshared ImGuiStyle ref_saved_style = ImGuiStyle(false);
    ...
    if (ImGui.BeginTabBar("##tabs", ImGuiTabBarFlags.None))
    {
        if (ImGui.BeginTabItem("Sizes"))
        {
            ...
            ImGui.SliderFloat2("ButtonTextAlign", &style.ButtonTextAlign, 0.0f, 1.0f, "%.2f"); ImGui.SameLine(); HelpMarker("Alignment applies when a button is larger than its text content.");
            ImGui.SliderFloat2("SelectableTextAlign", &style.SelectableTextAlign, 0.0f, 1.0f, "%.2f"); ImGui.SameLine(); HelpMarker("Alignment applies when a selectable is larger than its text content.");
            ...
            if (ImGui.Button("Export"))
            {
                ...
                for (int i = 0; i < ImGuiCol.COUNT; i++)
                {
                    ...
                    if (!output_only_modified || memcmp(&col, &_ref.Colors[i], sizeof!(ImVec4)) != 0)
                        ImGui.LogText("colors[ImGuiCol_%s]%*s= ImVec4(%.2ff, %.2ff, %.2ff, %.2ff);" ~IM_NEWLINE, name, 23 - cast(int)name.length, "", col.x, col.y, col.z, col.w);
                }
                ImGui.LogFinish();
            }
        ...
        if (ImGui.BeginTabItem("Fonts"))
        {
            ...
            for (int i = 0; i < atlas.Fonts.Size; i++)
            {
                ...
                if (font_details_opened)
                {
                    ...
                    const float surface_sqrt = ImSqrt(cast(float)font.MetricsTotalSurface);
                    ImGui.Text("Texture Area: about %d px ~%dx%d px", font.MetricsTotalSurface, cast(int)surface_sqrt, cast(int)surface_sqrt);
                    ...
                    if (ImGui.TreeNode("Glyphs", "Glyphs (%d)", font.Glyphs.Size))
                    {
                        ...
                        for (uint base = 0; base <= IM_UNICODE_CODEPOINT_MAX; base += 256)
                        {
                            ...
                            for (uint n = 0; n < 256; n++)
                                count += font.FindGlyphNoFallback(cast(ImWchar)(base + n)) ? 1 : 0;
                            if (count > 0 && ImGui.TreeNode(cast(void*)cast(size_t)base, "U+%04X..U+%04X (%d %s)", base, base + 255, count, count > 1 ? "glyphs" : "glyph"))
                            {
                                ...
                                for (uint n = 0; n < 256; n++)
                                {
                                    ...
                                    if (glyph)
                                        font.RenderChar(draw_list, cell_size, cell_p1, ImGui.GetColorU32(ImGuiCol.Text), cast(ImWchar)(base + n)); // We use ImFont::RenderChar as a shortcut because we don't have UTF-8 conversion functions available to generate a string.
...
struct ExampleAppConsole
{
    ...
    {
        ClearLog();
        memset(InputBuf.ptr, 0, sizeof(InputBuf));
        HistoryPos = -1;
    ...
    // Portable helpers
    int   Stricmp(string str1, string str2)         { int d; size_t index = 0; while (index < str1.length && (d = toupper(str2[index]) - toupper(str1[index])) == 0) { index++; } return d; }
    int   Strnicmp(string str1, string str2, int n) { int d = 0; size_t index = 0; while (n > index && index < str1.length && (d = toupper(str2[index]) - toupper(str1[index])) == 0) { index++;} return d; }
    string Strdup(string str)                             { size_t len = str.length; char[] buf = IM_ALLOC!char(len); IM_ASSERT(buf); memcpy(buf.ptr, cast(const void*)str.ptr, len); return cast(string)buf; }
    ...
    void    Draw(string title, bool* p_open)
    {
        ...
        ImGui.BeginChild("ScrollingRegion", ImVec2(0, -footer_height_to_reserve), false, ImGuiWindowFlags.HorizontalScrollbar); // Leave room for 1 separator + 1 InputText
        ...
        for (int i = 0; i < Items.Size; i++)
        {
            ...
            if (item[0..7] == "[error]")            { ImGui.PushStyleColor(ImGuiCol.Text, ImVec4(1.0f, 0.4f, 0.4f, 1.0f)); pop_color = true; }
            else if (item[0..2] == "# ")   { ImGui.PushStyleColor(ImGuiCol.Text, ImVec4(1.0f, 0.8f, 0.6f, 1.0f)); pop_color = true; }
        ...
        if (ImGui.InputText("Input", InputBuf, ImGuiInputTextFlags.EnterReturnsTrue|ImGuiInputTextFlags.CallbackCompletion|ImGuiInputTextFlags.CallbackHistory, &TextEditCallbackStub, cast(void*)&this))
        {
...
private void ShowExampleAppPropertyEditor(bool* p_open)
{
    ...
    struct funcs
    {
        static void ShowDummyObject(string prefix, int uid)
        {
            ...
            if (node_open)
            {
                __gshared float[8] dummy_members = [ 0.0f,0.0f,1.0f,3.1416f,100.0f,999.0f ];
                for (int i = 0; i < 8; i++)
                {
                    ...
                    else
                    {
                        ...
                        ImGui.AlignTextToFramePadding();
                        ImGui.TreeNodeEx("Field", ImGuiTreeNodeFlags.Leaf | ImGuiTreeNodeFlags.NoTreePushOnOpen | ImGuiTreeNodeFlags.Bullet, "Field_%d", i);
                        ImGui.NextColumn();
                        ImGui.SetNextItemWidth(-1);
                        if (i >= 5)
...
private void ShowExampleAppSimpleOverlay(bool* p_open)
{
    ...
    if (ImGui.Begin("Example: Simple overlay", p_open, (corner != -1 ? ImGuiWindowFlags.NoMove : ImGuiWindowFlags.None) | ImGuiWindowFlags.NoDecoration | ImGuiWindowFlags.AlwaysAutoResize | ImGuiWindowFlags.NoSavedSettings | ImGuiWindowFlags.NoFocusOnAppearing | ImGuiWindowFlags.NoNav))
    {
...
private void ShowExampleAppCustomRendering(bool* p_open)
{
    ...
    if (ImGui.BeginTabBar("##TabBar"))
    {
        ...
        if (ImGui.BeginTabItem("Canvas"))
        {
            ...
            if (ImGui.IsItemHovered())
            {
                if (!adding_line && ImGui.IsMouseClicked(ImGuiMouseButton.Left))
                {
                    points.push_back(mouse_pos_in_canvas);
                    adding_line = true;
                }
                if (ImGui.IsMouseClicked(ImGuiMouseButton.Right) && !points.empty())
                {
                    adding_line = adding_preview = false;
                    points.pop_back();
                    points.pop_back();
