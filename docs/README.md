...
I have tried to mimic the original api as close as posible while adapting certain things to the D programming language (e.q. using strings instead of "const char*" and arrays instead of pointers). Please look at the included demo programm in imgui_demo.d for example code. You can see the demo in action running `dub run d_imgui:glfw_vulkan`, which then compiles and runs the glfw-vulkan backend found in `examples_d` folder. All of ImGui is nothrow and @nogc.
